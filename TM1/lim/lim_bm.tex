...
\info{\note}{
\sym{$ x\to a$} omfatter de to tilfellene \sym{$ x\to a^+ $} og \sym{$ x\to a^- $}. Ofte vil disse være så like av natur at vi kan behandle \sym{$ x\to a $} som ett tilfelle.
}
\regv
...
\section{Kontinuitet}
\reg[Kontinuitet]{
Gitt en funksjon $ f(x) $ og en konstant $ c $. Hvis $ f(c) $ eksisterer, er $ f $ kontinuerlig for $ x=c $ hvis
\begin{equation}\label{kont}
	\lim\limits_{x\to c} f(x)=f(c) 
\end{equation}
Hvis \eqref{kont} er ugyldig, er $ f $ diskontinuerlig for $ x=c $.
}
...
Altså er $ g $ ikke kontinuerlig for $ x=2 $.
}
}
\newpage
