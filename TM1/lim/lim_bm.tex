...
\reg[Regneregler for grenseverdier]{
	Gitt funksjonene $f(x)$ og $g(x)$, hvor $\lim\limits_{x\to a}=b$ og $\lim\limits_{x\to a}=c$. Da er
	\begin{align}
		\lim\limits_{x\to a}(f+g)&=b+c \vn
		\lim\limits_{x\to a} fg &= bc
	\end{align}	
}
\eks[1]{
...
Dette betyr at 
\[ \lim\limits_{x\to 1} f(x) = \lim\limits_{x\to 1} x+3= 4 \] 
}
