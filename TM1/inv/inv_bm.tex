...
De fleste funksjonsverdier varierer. Beskrivelser av hvordan funksjonene varierer kaller vi beskrivelser av funksjonenes \outl{monotoniegenskaper}\index{monotoniegenskaper}.\regv
\reg[Voksende og avtagende funskjoner \label{voksogav}]{
Gitt en funksjon $ f(x) $.
\begin{itemize}
\item $ f $ er \outl{voksende}\index{voksende} på intervallet $ [a, b] $ hvis vi for alle $ {x_1, x_2 \in[a, b]}$ har at
...
\fig{voksende}
\item $ f $ er \outl{avtagende}\index{avtagende} på intervallet $ [a, b] $ hvis vi for alle $ {x_1, x_2 \in[a, b]}$ har at
...
\begin{itemize}
\item $ {f'\geq0} $ for $ {x\in(a, b)} \iff f $ er voksende for $ x\in[a, b] $
\item $ {f'\leq0} $ for $ {x\in(a, b)} \iff f $ er avtagende for $ x\in[a, b] $
\end{itemize}
Hvis henholdsvis \sym{$ \geq $} og \sym{$ \leq $} kan erstattes med \sym{$ > $} og \sym{$ < $}, er $ f $ strengt voksende/strengt avtagende. 
} \regv
...
\reg[Konvekse og konkave funksjoner \label{konvkonk}]{
	...
	Hvis hele linja mellom $ (a, f(a)) $ og $ (b, f(b)) $ ligger over grafen til $ f $ på intervallet $ [a, b] $, er $ f $ \outl{konveks}\index{konveks} for $ x\in[a, b] $.	
	\begin{figure}
		\centering
		\includegraphics[]{\figp{konv}}
	\end{figure}	
	Hvis hele linja mellom $ (a, f(a)) $ og $ (b, f(b)) $ ligger under grafen til $ f $ på intervallet $ [a, b] $, er $ f $ \outl{konkav}\index{konkav} for $ x\in[a, b] $.
	\begin{figure}
		\centering
...
\spr{
Et maksimum/minimum blir også kalt en \outl{maksimums-verdi}/\outl{minimumsverdi}\index{maksimumsverdi}\index{minimumsverdi}. \vsk

Gitt en funksjon $f(x)$. Hvis ${D_f=[a, b]}$, sier vi at $a$ og $b$ er ende-punktene til $f$.
}
\reg[Ekstremalverdi og ekstremalpunkt \label{extremumpoints}]{\index{ekstremalpunkt}\index{ekstremalverdi}
	...
	\begin{itemize}
		\item $ f(c) $ en \outl{ekstremalverdi}\index{ekstremalverdi} for $ f $.
		\item $ c $ et \outl{ekstremalpunkt}\index{ekstremalpunkt} for $ f $. Nærmere bestemt et maksimalpunkt/minimumspunkt for $ f $.
		\item $ (c, f(c)) $ et \outl{toppunkt/bunnpunkt}\index{toppunkt}\index{bunnpunkt} for $ f $.
	\end{itemize}
}
\reg[Kritiske punkt]{
Et tall $ c $ er et \outl{kritisk punkt}\index{kritisk punkt} for en funksjon $ f(x) $ hvis ett av følgende gjelder: 
\begin{itemize}
	\item $f'(c)=0$.
...
\spr{
	...
	Hvis $f'(c)=0$, kalles $c$ også et \outl{stasjonært punkt}\index{stasjonært punkt}. 
}
\info{Obs!}{
...
\fork{\ref{andredertest} \andredertest}{
	...
	Altså må $ {f'(c+h)} $ være positiv når $ h $ går mot 0 fra venstre og negativ når $ h $ går mot 0 fra høgre. Dermed skifter $ f' $ fortegn i $ c $, som da må være et lokalt maksimalpunkt for $ f $. Tilsvarende må $ c $ være et lokalt minimumspunkt for $ f $ hvis $ {f(c)=0} $ og $ {f''(c)>0} $.
}

\reg[Infleksjonspunkt og vendepunkt]{\index{infleksjonspunkt}\index{vendepunkt}
	For en kontinuerlig funksjon $ f(x) $ har vi at
	\begin{itemize}
		\item Hvis $ {f''(c)=0} $ og $ f'' $ skifter fortegn i $ c $, er $ c $ et \outl{infleksjonspunkt}\index{infleksjonspunkt} for $ f $.
		\item Hvis $ c $ er et infleksjonspunkt for $ f $, er $ (c, f(c)) $ et \outl{vendepunkt}\index{vendepunkt} for $f$.
...
\rowcolor{green!15}		$ A=(-14, -1456) $ & absolutt bunnpunkt \\ \hline
\rowcolor{green!15}		$ -14 $ & ekstremalpunkt; absolutt minimum \\ \hline
\rowcolor{green!15}		$ -1456 $& absolutt minimum \\ \hline
\rowcolor{Cyan!15}		$ B=(-6, 400) $ & lokalt toppunkt \\ \hline	\rowcolor{Cyan!15}		
		$ -6 $ & ekstremalpunkt; lokalt maksimalpunkt \\ \hline \rowcolor{cyan!10}	
		$ 400 $ & lokalt maksimum \\ \hline
\rowcolor{green!15}		$ C=(-1, -286) $ & vendepunkt  \\ \hline
\rowcolor{green!15}		$ -1 $ & infleksjonspunkt \\ \hline 
\rowcolor{Cyan!15}		$ D = (8, -972) $ & lokalt bunnpunkt \\ \hline
\rowcolor{Cyan!15}		$ 8 $ & ekstremalpunkt; lokalt minimumspunkt \\ \hline
\rowcolor{Cyan!15}		$ -972 $ & lokal minimum \\ \hline
\rowcolor{green!15}		$ E= (16, 884) $ & absolutt maksimum \\ \hline
\rowcolor{green!15}		$ 16 $ & ekstremalpunkt; absolutt maksimumspunkt \\ \hline
\rowcolor{green!15}		$ 884 $ & absolutt maksimum \\ \hline
...
\spr{
Et nullpunkt til en funksjon kalles også \outl{roten}\index{rot} til funksjonen.
}
\newpage
\section{Asymptoter} \index{asymptote}
...
\begin{itemize}
	...
	\item Hvis $ {\lim\limits_{x\to c} f(x)=\pm \infty}  $, er $ c $ en \outl{vertikal asymptote} for $ f $.\index{vertikal asymptote}
\end{itemize}
}
\eks[]{
...
Gitt en funksjon $ f(x) $. Hvis ${\lim\limits_{x\to \infty} f(x)=c}$ eller ${\lim\limits_{x\to -\infty} f(x)=c}$, er $ {y=c} $ en \outl{horisontal asymptote}\index{horisontal asymptote} for $ f $ 
}
\eks[]{
...
\reg[Injektive funksoner]{
	Gitt en funksjon $ f(x) $. Hvis $f(x_1)\neq f(x_2)$ for alle $x_1, x_2\in[a, b] $, er $ f $ \outl{injektiv}\index{injektiv} på dette intervallet.
}
\spr{
	Et annet ord for injektiv er \outl{én-entydig}\index{én-entydig}.
}
...
\[ x(f)=\frac{f-1}{2} \]
Denne funksjonen kalles den \outl{omvendte funksjonen}\index{omvendt funksjon} til $ f $. Setter vi uttrykket til $ f $ inn i uttrykket til $ x(f) $, får vi nødvendigvis $ x $:
\alg{
