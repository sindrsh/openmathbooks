...
\begin{itemize}
\item Hvis $ {f'\geq0} $ for $ {x\in(a, b)} $, er $ f $ \outl{voksende} for $ x\in[a, b] $
\item Hvis $ {f'\leq0} $ for $ {x\in(a, b)} $, er $ f $ \outl{avtagende} for $ x\in[a, b] $
\end{itemize}
...
Gitt en deriverbar funksjon $ f(x) $ og tallene $a$, $b$ og $c$, hvor $ [a, b] \in D_f $ og $a<c<b$.
...
\regv
\spr{
	Det som blir beskrevet i punkt (ii) og (iii) over omtales ofte som at ''$ f $ skifter fortegn i $ c $''. 
} \vsk
...
\fork{\ref{monder}\,\monder}{
	...
	Følgelig er $ {f(x_2)\geq f(x_1)} $, og av \dref{voksogav} er da $ f $ voksende på intervallet $ (a, b) $.
}\vsk
...
\fork{\ref{fderekstr} \fderekstr}{
	...
	\textbf{Punkt (ii)} \os
	Hvis $ {f'>0} $ på intervallet $ (a, c) $, har vi av \rref{monder} at $ f $ er sterkt voksende på intervallet. Hvis $ {f'<0} $ på $ (c, b) $, er $ f $ sterkt avtagende på intervallet. Dette må nødvendigvis bety at $ {f(c)\geq f(x)} $ for $ {x\in(a, b)} $, og da er $ c $ et maksimumspunkt.\vsk
