...
\spr{
Koordinater uttrykt ved én eller flere variabler (parametre) kalles en \outl{parameterisering}\index{parameterisering}. En vektorfunksjon er altså en parameterisering av en kurve.
}
...
\reg[Linje som vektorfunksjon]{
	En linje $ \vec{l}(t) $ som går gjennom punktet $ {A=(x_0, y_0,)} $ og har retningsvektor $ {\vec{r}=[a, b] }$ er gitt ved
	\nn{
	\vec{l}=A+t\vec{r}=[x_0 + at, y_0+ bt]	\label{linje}
	}
}
\eks[1]{
