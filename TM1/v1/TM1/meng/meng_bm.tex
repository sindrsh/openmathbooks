...
}\index{mengde}, og et tall som er en del av en mengde kalles et \outl{element}\index{element}. Mengder kan inneholde et endelig\\ antall elementer og de kan inneholde uendelig mange elementer. \regv
\regdef[Mengder]{
For to reelle tall $ a $ og $ b $, hvor $ a\leq b $, har vi at
\begin{center}
	\begin{tabular}{c l}
		$ [a, b] $ & er mengden av alle tall større eller lik $ a $ \\
		&og mindre eller lik $ b $. \br
		$ (a, b] $ & er mengden av alle tall større enn $ a $ \\
		&og mindre enn eller lik $ b $.\br 
	...
	\end{tabular}
\end{center}
$ [a, b] $ kalles et \outl{lukket intervall}\index{intervall}\index{intervall!lukket}, $ (a, b) $ kalles et \outl{åpent \\intervall}\index{intervall!åpent}, og både $ (a, b] $ og $ [a, b) $ kalles \outl{halvåpne intervall}\index{intervall!halvåpent}. \\ $a$ og $b$ er \outl{endepunktene} til intervallet.\vsk
...
At $ x $ er utelatt av en mengde $ M $ skrives som $ M \setminus \{x\} $.\vsk

At både $x$ og $y$ er et element i en mengde $M$ skrives som \\$x, y\in M$
}
\spr{
$ x \in M$ uttales ''$ x $ inneholdt i $ M $''.\vsk

Mange tekster bruker \sym{$ \langle $} i stedet for \sym{$ ( $} for å indikere åpne (eller halvåpne) intervall.\vsk

Hvis $I=(a, b)$ og $c\in I$, sier vi at $I$ er et åpent intervall om $c$.
}
\info{\note}{
Når vi heretter i boka definerer et intervall beskrevet av $ a $ og $ b $, tar vi det for gitt at $ a $ og $ b $ er tall, og at $ a\leq b $.
}
...
\eks[2]{
I uttrykket $ 0\ ?\  x\  ?\  1 $, erstatt begge \sym{?} med ulikhetssymbol slik at uttrykket gjelder for alle $ {x\in M} $, og avgjør om 1 er inneholdt i $ M $.
\abc{
\item $ M = [0, 1] $
\item $ M = (0, 1] $
\item $ M = [0, 1) $
...
\spr{
Når vi skriver ''tallet $x$'' eller ''konstanten $k$'', tas det i denne boka for gitt at $x$ og $k$ er reelle tall. Når vi skriver ''funksjonen $f(x)$'', tar vi det for gitt at $x$ er et reelt tall og at alle andre symboler i funksjonsuttrykket er reelle konstanter.
}
...
\eks[]{
At $ x\geq2 $ kan vi skrive som $ x\in[2 ,\, \infty) $.\vsk

At $ x<-7 $ kan vi skrive som $ x\in(-\infty ,-7) $.
}
\spr{
...
Gitt en funksjon $ f(x) $. \vsk

Mengden som utelukkende inneholder alle $ x $-verdier som kan anvendes i funksjonsuttrykket til $f$, er \outl{definisjonsmengden}\index{definisjonsmengde} til $ f $. Denne mengden skrives som $ D_f $. \vsk

Mengden som utelukkende inneholder alle $ f $-verdier for $ {x\in D_f} $, er \outl{verdimengden}\index{verdimengde} til $ f $. Denne mengden skrives som $ V_f $. 
}
\eks[1]
{ \label{mengeks1}
Figuren under viser grafen til $ {f(x)=2x-1} $, hvor $ {D_f=[1, 3]} $. Da er ${ V_f=[1, 5]} $. \vs
\fig{defmengeks1}
}
\eks[2]{
Figuren under viser grafen til $ f(x)={\frac{1}{x}} $, hvor $ {D_f}=[-3, -1] \cup [2, 5] $. Da er $ {V_f=\left[-1, -\frac{1}{3}\right]\cup \left[\frac{1}{5},\frac{1}{2}\right]} $. \vs
\fig{defmengeks2}
}
\newpage
\info{\note}{
...
En funksjon kan ha flere uttrykk som gjelder ved forskjellige vilkår. Da sier vi at funksjonen har \outl{delt forskrift}\index{delt forskrift}. La oss for eksempel definere en funksjon $ f(x) $ slik:
\alg{
