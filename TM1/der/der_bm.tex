...
\spr{
Et annet ord for en \textit{tangeringslinje} er en \outl{tangent}. \os
...
Hvis vi for en funksjon $f(x)$ har at $f'(a)$ eksisterer, sier vi at $f$ er \outl{deriverbar} for $x=a$.
...
Den deriverte funksjonen til $f$ kalles også den \outl{førstederiverte} funksjonen av $f$. \vsk

En funksjon $f(x)$ som er deriverbar for alle ${x\in D_f}$ kaller vi en \outl{deriverbar funksjon}.
}
\info{Den deriverte av høyere orden}{
Gitt en funksjon $f$. Hvis vi deriverer $f'$, får vi den \outl{andrederiverte} funksjonen av $f$, som vi skriver som $f''$. Altså er ${(f')'=f''}$. På samme måte er ${(f'')'=f'''}$, ${(f''')'=f''''}$ og så videre.
}
\newpage
...
\begin{align*}
	...
	\frac{f(b)-g(x)}{b-x}&=[u'(x)+v_u(b)][g'(t)+v_g(b)]
\end{align*}
Følgelig har vi at
\begin{align*}
...
Vi setter $ u=x^r $. Av \refunnbr{kjerne}{kjerneregelen} (\rref{kjerne}) har vi da at
\alg{
\frac{r}{x}&=	(\ln u)'\\
	&=\frac{1}{u} u' \\
	&=\frac{1}{x^r}(x^r)' 
