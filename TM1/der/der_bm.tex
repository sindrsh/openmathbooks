...
\spr{
Et annet ord for en \textit{tangeringslinje} er en \outl{tangent}. \vsk
...
\reg[Den deriverte funksjonen og den deriverte \label{fderandder}]{
Hvis både $f(x)$ og $f'(x)$ er kontinuerlig for ${x=a}$, er $f$ deriverbar for ${x=a}$.
}
...
\[ f(x, y)= x^2+y^3 \]
en \outl{flervariabel funksjon}, avhengig av både $ x $ og $ y $. I dette tilfellet kan vi bruke skrive \sym{$ \frac{\text{d}}{\text{d}x} f$} for å indikere derivasjon med hensyn på $ x $, og \sym{$ \frac{\text{d}}{\text{d}y}f $} for å indikere derivasjon med hensyn på $ y $. Leseren må gjerne forklare for seg selv hvorfor følgende stemmer:
...
\fork{\ref{fderandder} Den deriverte funksjonen og den deriverte}{
Vi har at
\[ f'(a)= \lim\limits_{h\to 0} \frac{f(a+h)-f(a)}{h} \]
Siden $f$ er kontinuerlig for $x=a$, går telleren mot 0.
}
