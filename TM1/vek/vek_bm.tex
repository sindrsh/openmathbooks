...
\sv \vsb
\begin{align*}
	t(\vec{u}+\vec{v})-(t\vec{u}+\vec{v}) = t\vec{u}+t\vec{v}-t\vec{u}-\vec{v}=(1-t)\vec{v}
\end{align*}
}
...
\reg[Linje som vektorfunksjon]{
	En linje $ \vec{l}(t) $ som går gjennom punktet $ {A=(x_0, y_0,)} $, og som har retningsvektor $ {\vec{r}=[a, b] }$, er gitt som
	\nn{
	\vec{l}=[x_0 + at, y_0+ bt]	\label{linje}
	}
}
\eks[1]{
En linje $\vec{l}(t)$ går gjennom punktene $(3, 6)$ og $(5, 13)$. Finn uttrykket for $\vec{l}$. Skriv uttrykket både som en vektorfunksjon og på parameterisert form.
