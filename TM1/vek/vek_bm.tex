...
\newpage
\info{Nullvektoren}{
I forkant av \rref{vinkrett} har vi bare argumentert for at\\ ${\vec{u}\perp\vec{v}\Rightarrow \vec{u}\cdot\vec{v}=0} $. For å rettferdiggjøre vilkåret som går begge veier i \eqref{vinkretteq}, må vi vise at hvis ${ \vec{u}\cdot\vec{v} =0}$, så er $\vec{u}\perp\vec{v}$.\vsk
...
\spr{
Koordinater uttrykt ved én eller flere variabler (parametere) kalles en \outl{parameterisering}\index{parameterisering}. En vektorfunksjoner er altså en parameterisering av en kurve.
}
...
\reg[Linje som vektorfunksjon]{
	En linje $ \vec{l}(t) $, og som går gjennom punktet $ {A=(x_0, y_0,)} $ og har retningsvektor $ {\vec{r}=[a, b] }$, er gitt som
	\nn{
...
\fig{plin}
Den korteste avstanden fra \textit{B} til linja er høyden \textit{h} i trekanten formet av $ \vec{r} $ og $ \vv{AB} $. Arealet til denne trekanten er gitt som \eqref{det2artrk}:
...
På lignende måte kan det vises at \eqref{det2arparl} gjelder for alle $a, b, c, d\in\mathbb{R} $. \vsk

\mers{\eqref{det2arparl} kan også vises veldig kortfattet ved å anvende trigonometri. Dette er gitt som en oppgave i \tmto.}
}
\end{document}