...
Gitt en funksjon $ \vec{r}(t) $ som er kontinuerlig og to ganger deriverbar for alle $ t\in\mathbb{R} $, og hvor $ f''(t), g''(t)\neq0 $. Vi innfører punktene
\nn{
...
Vi har at
\alg{
	\vv{BA}&= [f_t-f_{b}, g_t-g_{b}] \br \vv{AC}&=[f_c-f_t, g_c-g_t]
}
La $ B_m $ og $ C_m $ være midptunktene til henholdsvis  (sekantene) $ AB $ og $ AC $. Da er
\nn{
	B_m = \frac{1}{2}(A+B)\qquad,\qquad C_m = \frac{1}{2}(A+C)
}
$ [g_t-g_b,f_b-f_t] $ er en normalvektor for $ \vv{BA} $, dette betyr at midtnormalen $\bm l_1 $ til sekanten $ AB $ kan parameterisere som
\nn{
{\bm l_1(t)}=B_m+[g_t-g_b,f_b-f_t] t \label{l1}
}
Tilsvarende er midtnormalen $\bm l_2 $ til sekanten $ AC $ parameterisert ved
\[ {\bm l_2(q)}=C_m+[g_t-g_c,f_c-f_t] q \]
