...
\abc{
\item Hva er verdien til $ f(t) $ og $ f(s) $? Grunngi svaret uten bruk av utregning.
...
\item Regn ut $ st $. Hvis $ f(0)=2 $ og $ st=3 $, hva er da verdiene til $ a $ og $ c $?
}
...
\abch{
\item $ \dfrac{x^4-2x^3+3x^2+5}{x^3+x} $
\item $ \dfrac{-7x^3-9x^2+x}{-4x^2+3} $
}
\abcs{3}{
...
\abchs{4}{
\item $ 3\lg x+2\lg x^2 + \lg \frac{1}{x^9}=2 $
}
...
Faktoriser uttrykket $y^2-9k^5y-k^2y+9k^7$.
...
\item Vis at
\[ \frac{1}{2}(p+q)(q^2-p^2)-pq(q-p)-\frac{1}{3}(q^3-p^3)=\frac{a}{6}(q-p)^3 \]
}
\newpage
\grubop{opggeoher}
