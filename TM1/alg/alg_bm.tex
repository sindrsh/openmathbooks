...
For to reelle tall $ a $ og $ b $ er \index{kvadratsetningene}
\alg{
...
\spr{
$ (a+b)^2 $ og $ (a-b)^2 $ kalles \outl{fullstendige kvadrat}\index{fullstendig kvadrat}.\vsk	
	
3. kvadratsetning kalles også \outl{konjugatsetningen}\index{konjugatsetningen}.\vsk

Alle kvadratsetningene er \textit{identiteter}. En \outl{identitet}\index{identitet} er en likning som er oppfylt uansett hvilke verdier man gir variablene i likningen. 
}
\eks[1]{
...
En funksjon $f(x)$ består gjerne av uttrykk som har positive og negative verdier for forskjellige verdier av $x$. For å få et tydeligere bilde av når $f$ er positiv eller negativ, kan vi lage et \outl{fortegnsskjema}\index{fortegnsskjema}.
\eks[1]{
	Løs ulikheten
	\[ x^2-8x-9\leq0 \] \vs
	\sv
...
Skjemaet viser følgende:
\begin{itemize}
	\item Uttrykket $ x-3 $ er negativt når  $ x<3 $, lik 0 når $ x=3 $, og positivt når $ x>3 $.
	...
	\item Siden $f=\frac{(x-3)(x+2)}{x-4}$, har vi at \vspace{-5pt}
	\begin{center}
		\begin{tabular}{l}
...
\reg[Andregradslikning med konstantledd]{
	...
	\end{equation}
	Hvis $b^2-4ac<0$, har ikke likningen noen reell løsning.
}
\eks[1]{
	\abc{
...
\]
Den samme tankegangen kan brukes for brøker med polynomer\footnote{Se \refved{vdlfunknavn}}, og da kalles det \outl{polynomdivisjon}\index{polynomdivisjon}. 
\regv
...
\begin{enumerate}[label=\roman*)]
	\item  Leddet med den høyste graden av $ x $ i dividenden er $ 2x^2 $. Dette uttrykket kan vi få ved å multiplisere divisoren med $ 2x $. Vi skriver $ 2x $ til høyre for likhetstegnet, og subtraherer $ 2x(x+5)=2x^2+10x $.
	\item Differansen fra punkt i) er ${-7x-4}$. Vi kan få leddet med den høyeste gradenen av $ x $ ved å multiplisere divisoren med $ -7 $. Vi skriver $ -7 $ til høyre for likhetstegnet, og subtraherer $ -7(x+5)=-7x-35 $. 
...
\st{\[ e=2.718281828... \]}
De mest fascinerende egenskapene til dette tallet kommer til syne når man undersøker funksjonen ${f(x)= e^x} $. Dette er en eksponentialfunksjon\footnote{Se \refved{vdlfunknavn}.} som er så viktig at den rett og slett går under navnet \\\outl{eksponentialfunksjonen}\index{eksponentialfunksjonen}. Denne funksjonen skal vi undersøke nærmere i \refved{eulerstallfork} og \refkap{Derivasjon}.
\fig{ekspfunk}
\section{Logaritmer}
I \mb\, så vi at potenstall består av et grunntall og en eksponent. En \outl{logaritme}\index{logaritme} er en matematisk operasjon relativ til et tall. Hvis en logaritme er relativ til grunntallet til en potens, vil operasjonen resultere i ekspontenten.\vsk
...
\begin{equation}
	\log_a a^m = m
\end{equation}
For $m>0$ kan vi alternativt skrive
\begin{equation}
	m=a^{\log_{a} m}
\end{equation}
}
...
\newpage
\spr{
	\sym{$ \log_{10}$} skrives ofte som \sym{$ \log $} eller \sym{$\lg$}, mens \sym{$ \log_e $} skrives ofte som \sym{$ \ln $} eller (!) \sym{log}. Når man bruker digitale hjelpemidler til å finne verdier til logaritmer er det derfor viktig å sjekke hva som er grunntallet. I denne boka skal vi skrive \sym{$\log_{10}$} som \sym{$\log$}, og \sym{$ \log_e $} som \sym{$ \ln $}.	\os
	
	Logaritmen med $ e $ som grunntall kalles den \outl{naturlige logaritmen}\index{logaritme!naturlig}. \os
	
	Uttrykket $(\ln x)^2$ skrives gjerne som $\ln^2 x$.
}
...
\reg[Logaritmeregler]{ \label{logaritmrules}
...
\fork{(\ref{logaritmrules}) Logaritmerregler}{
