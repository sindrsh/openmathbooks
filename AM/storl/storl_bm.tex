...
\eks[1]{
	...
	\sv
	Vi skriver tabellen vår med L i midten, og ser at vi må\\ \textbf{tre ruter til venstre} for å komme oss fra \colr{mL} til \colb{L}:
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|c|c}
			kilo &
			hekto &
...
\eks[2]{
	...
	\sv
	Vi skriver tabellen vår med 'g' i midten, og ser at vi må\\ \textbf{fire ruter til høyre} for å komme oss fra \colr{hg} til \colb{cg}:
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|c|c}
			kilo &
			\color{red}hekto &
...
\eks[3]{
	...
	\sv
	Vi skriver tabellen vår med 's' i midten, og ser at vi må\\ \textbf{tre ruter til høyre} for å komme oss fra \colr{s} til \colb{ms}:
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|c|c}
			kilo &
			hekto &
...
\eks[4]{
	Skriv om 12\,500\,dm til antall mil.
...
\regdef[Kilopris \label{kilopris}]{
...
\outl{Tetthet} gir forholdet mellom en masse og et volum.
\begin{equation}\label{tettheteq}
	\text{tetthet}=\frac{\text{masse}}{\text{volum}}
\end{equation}
Alternativt kan vi skrive
\begin{equation}\label{tetthetalteq}
	\text{masse}=\text{tetthet}\cdot\text{volum}
\end{equation}
Vanlige benevninger for tetthet er 'kg/m$ ^3 $' og 'g/cm$ ^3 $'	
}
\spr{
I dagligtale kaller vi gjerne 'masse' for 'vekt', men i fysikk er dette to forskjellige størrelser.
}
\regdef[Effekt \label{effekt}]{
	...
	\end{equation}
	Vanlige benevninger for effekt er 'W' og 'kW'. 'W' står for 'Watt'. 'W' kan også skrives som 'J/s', hvor 'J' står for 'Joule'. 'Joule' er en enhet for energi.
}
\newpage
\info{\note}{
I \rref{kilopr}\,-\,\rref{effekt} er det antatt at størrelsen som kan uttrykkes ved en brøk er en konstant, derav navnet \outl{proporsjonalitetskonstant}. Men det er ikke alltid slik; lar vi en stein falle fra en høyde, vil den ikke ha den samme farten hele tiden. Si at en stein har brukt $t$ sekunder på å falle $s$ meter. Farten vi finner ved å regne ut $\frac{t}{s}$ er denne: \textsl{Den konstante farten som trengs for å reise $s$ meter i løpet av $t$ sekunder}.
}
\section{Regning med forskjellige benevninger \label{regnmforbenvn}}
Når vi skal utføre regning med størrelser som har benevning, er det helt avgjørende at vi passer på at benevningene er like. \regv
...
\eks[2]{
	...
	Her har vi to forskjellige enheter for tid involvert; h (timer) og min (minutt). Da må vi enten gjøre om farten til antall km/min eller tiden til antall h. Vi velger å gjøre om antall min til antall h\footnote{Husk at $ 60\enh{min}=1\enh{h}. $}:
	\[ 45\enh{min}=\frac{45}{60}\enh{h} =\frac{3}{4}\enh{h} \]
	Av \rref{fart} har vi at
