...
I \mb\, har vi sett hvordan brøker er definert ut fra en inndeling av 1. I hverdagen bruker vi også brøker for å snakke om inndelinger av en helhet: \vs
\begin{figure}
	\centering
...
\begin{center}
	\begin{enumerate}[label=({\roman*})]
		...
		\item Helheten er et kvadrat. $ \frac{1}{2} $ av kvadratet er rødt.
...
\fig{br2}
Når vi sier ''$\frac{\colb{2}}{\colg{3}}$ av \colr{12}'' mener vi at vi skal
\st{\begin{enumerate}[label=(\roman*)]
	\item dele \colr{12} inn i \colg{3} like grupper.
...
For å finne $ \frac{2}{3} $ av 12, delte vi 12 med 3, og ganget kvotienten med 2. Dette er det samme som å gange $ 12 $ med $ \frac{2}{3} $ (se \mb).\regv
...
\spr{
Deler av en helhet blir også kalt \outl{andeler}.
}
\section{Prosent} \index{prosent}
Brøker er ypperlige til å oppgi andeler av en helhet fordi de gir et raskt bilde av hvor mye det er snakk om. For eksempel er det lett å se (omtrent) hvor mye $ \frac{3}{5} $ eller $ \frac{7}{12} $ av en kake er. Men om vi skal avgjøre hvilke andeler som utgjør \textsl{mest}, er det best om brøkene har samme nevner. \os

Når andeler oppgis i det daglige, er det vanlig å bruke brøker med 100 i nevner. Slike brøker er så mye brukt at de har fått sitt eget navn og symbol. \regv

\reg[Prosenttall \label{prosenttall}]{
\[ a\,\% = \frac{a}{100} \]
}
\spr{
\sym{\%} er symbolet for ordet \outl{prosent}. Ordet kommer av det latinske \textit{per centum}, som betyr \textit{per hundre}.
}
\eks[1]{ 
\[ 43\,\%=\frac{43}{100} \]
}
\eks[2]{ 
\[ 12,7\,\% = \frac{12,7}{100} \]
\mers{Det er kanskje litt uvant, men ikke noe galt med å ha et desimaltall i teller (eller nevner)}.
}
...
\sv \vso
\abc{
\item $ \dfrac{34}{100}=34\,\% $
\item $ \dfrac{203}{100}=203\,\% $
}
}
\spr{
Verdien til et prosenttall kalles \outl{prosentfaktoren}.
}
\newpage

\eks[5]{
Gjør om tallene til prosenttall.\os
\abch{
	\item 97
	\item 0,45
	\item $ \dfrac{1}{8} $
...
\sv
\abc{
\item $97=\dfrac{9700}{100}=9700\,\%$
\item $0,45=\dfrac{45}{100}=45\,\%$
\item $\dfrac{1}{8}=0,125=\dfrac{12,5}{100}=12,5\,\%$
}
}
\eks[6]{
	Finn 50\% av 800.
...
\info{Tips}{Å dele med 100 er såpass enkelt, at vi gjerne kan uttrykke prosenttall som desimaltall når vi gjør utregninger. I \textsl{Eksempel 6} over kunne vi ha regnet slik:
\[ 2\% \text{ av } 7,4 = 0,02\cdot 7,4 =0,148 \]
}
\info{Promille}{
Noen ganger er det bedre å bruke brøker med 1000 i nevner for å oppgi andeler. Da brukes symbolet \sym{\textperthousand}, som uttales \outl{promille}.
\[ a\,\text{\textperthousand} = \frac{a}{1000} \]
Alle regler om prosentregning kan også brukes ved promille-regning, så lenge man erstatter 100 med 1000.
}
\newpage
...
\eks[1]{
Hvor mange prosent utgjør \colb{340} av \colg{400}? 

\sv \vsb
\[ \colb{340} \cdot \frac{100}{\colg{400}}=85 \]
...
\alg{
\text{200 redusert med 60\%} &= 200-200\cdot60\% \\
&= 200\cdot(1-60\%) \\
&= 200\cdot 40\%
}
Dette betyr at
...
\eks[1]{ \label{vekstfakteks}
	...
	\sv
	$ 100\%-\colr{70}\%=\colg{30}\% $, altså er
	\alg{
		\colb{210}\text{ redusert med } \colr{70}\% &=\colg{30}\% \text{ av } \colb{210} \br &=\frac{\colg{30}}{100}\cdot\colb{210}\br
		&=63 
	}
}
\eks[2]{
...
\spr{
\textit{Rabatt} er en pengesum som skal trekkes fra en pris når det gis tilbud. Dette kalles også et \textit{avslag} på prisen. Rabatt oppgis enten i antall\enh{kr}oner eller som prosentdel av prisen. \vsk
...
\parbox[l][][l]{0.485\linewidth}{
	På bildet står det at prisen på øreklokkene er 999,20\enh{kr} \textsl{ekskludert} mva. og 1\,249 \textsl{inkludert} mva. For øreklokker er mva. 25\% av prisen. \os
...
\[ \text{125\% av 999,20}=\frac{125}{100}\cdot999,20= 1249 \]
...
\reg[Prosentvis endring med vekstfaktor \label{vekstfaktendr}]{
...
\sv  \vs
\abc{
	\item Siden det er 20\% rabatt, må vi betale $ 100\%-20\%= 80\% $	av originalprisen. Vekstfaktoren er derfor 0,8. 
...
$ 100\%+15\%= 115\% $
av prisen ekskludert mva. Vekstfaktoren er derfor 1,15.
\item Vi har at
\[ 1,15\cdot 9,80=11,27 \]
Sjokoladen koster 11,27\enh{kr}  inkludert mva.
}
} \vsk
Vi kan også omskrive likningen\footnote{Se \refkap{LigningerAM} for hvordan skrive om likninger.} fra \rref{vekstfaktendr} for å få et uttrykk for vekstfaktoren: \regv

\reg[Vekstfaktor II \label{vekstfaktfrm}]{ 
...
\end{enumerate}
\reg[Prosentvis endring I \label{proendra}]{
...
\[ \text{antall prosent 8 utgjør av 20}=8\cdot\frac{100}{20}=40 \]
...
\reg[Prosentvis endring II \label{proendrb}]{
	\[ \text{prosentvis endring}=100\left(\text{vekstfaktor}-1\right) \]
	
}
\newpage
\eks[1]{
	I 2019 tjente du 12\,000\enh{kr} og i 2020 tjente du 10\,000\enh{kr}. Finn den prosentvise endringen i din inntekt, med inntekten i 2019 som utgangspunkt.
...
\subsection{Prosentpoeng}
Ofte snakker vi om mange størrelser samtidig, og når man bruker ordet \textit{prosent}, kan setninger bli veldig lange og knotete hvis man også snakker om forskjellige originalverdier (utgangspunkt). For å skille mellom originalverdier har vi begrepet \outl{prosentpoeng}\index{prosentpoeng}.\os


Tenk at et par solbriller først ble solgt med 30\,\% rabatt av \\ originalprisen, og etter det med 80\,\% rabatt av originalprisen. Siden $80\,\%-30\,\%=50\,\%$, sier vi at rabatten har økt med 50 \textsl{prosentpoeng}.\os
...
\textit{Forklaring:} Rabatten er en størrelse vi regner ut fra originalprisen til solbrillene. Når vi bruker \textsl{prosentpoeng}, viser vi til at \textbf{originalprisen fortsatt er utgangspunktet} for den kommende prosentregningen:\os 

Når prisen er 1\,000\enh{kr}, starter vi med $ {1\,000\enh{kr}\cdot0,3=300\enh{kr}} $  i rabatt. Når vi legger til 50 \textsl{prosentpoeng} i rabatt, legger vi til 50\% av originalprisen i rabatt. Altså $ {1\,000\enh{kr}\cdot0,5=500\enh{kr}} $ mer i rabatt. Totalt blir det 800\,kr i rabatt, som utgjør $ 80\% $ av originalprisen.
}
\st{''Rabatten var først 30\%, så økte rabatten med 50\%. Da ble rabatten 45\%.''\vsk

\textit{Forklaring:} Rabatten er en størrelse vi regner ut fra originalprisen til solbrillene, men når vi snakker om en prosentvis økning av rabatten, er \textbf{det rabatten som er utgangspunktet} for den kommende prosentregningen: \os

Når prisen er 1\,000\,kr, starter vi med 300\,kr i rabatt. Videre er
...
\algv{
	...
	\text{verdi etter 3. endring}&=\overbrace{2\,000\cdot1,10\cdot1,10}^{\text{2\,420}}\cdot1,10=2\,662 
}
...
\reg[Gjentatt vekst eller nedgang \label{progjen}]{
...
\sv
$20\%=0,2$. Siden det er snakk om en økning, er vekstfaktoren\\ $1+0,2=1,2$. Da er
\[ \text{ny verdi} = 10\,000\cdot 1,2^6=29\,859,84  \]
}
\newpage
\eks[2]{
	...
	\sv
	${12\%=0,12}$. Siden det er snakk om en reduksjon, er vekstfaktoren ${1-0,12=0,88}$. Da er
	\[ \text{ny verdi}=300\,000\cdot0,88^4\approx179\,909 \]
	Marion forventer altså at bilen er verdt ca. 179\,909\enh{kr} om fire år.
}
...
I denne sammenhengen kalles 0,2 \outl{forholdstallet}.\regv
...
\eks[1]{
	I en klasse er det 10 håndballspillere og 5 fotballspillere.
	\abc{
\item Finn forholdet mellom antall håndballspillere og fotballspillere. Finn også forholdstallet.

\item Finn forholdet mellom antall fotballspillere og håndballspillere. Finn også forholdstallet.
} 
\sv
\abc{
\item Forholdet mellom antall håndballspillere og fotballspillere er
\[ \frac{10}{5}=\frac{2}{1} \]
Forholdstallet er 2.
\item Forholdet mellom antall fotballspillere og håndballspillere er
\[  \frac{5}{10}=\frac{1}{2} \]
Forholdstallet er $0,5$.
}
}

\subsection{Målestokk}
I \mb\,(s.145\,-\,149) har vi sett på formlike trekanter. Prinsippet om at forholdet mellom samsvarende sider er det samme, kan utvides til å gjelde de fleste andre former, som f. eks. firkanter, sirkler, prismer, kuler osv. Dette er et fantastisk prinsipp som gjør at små tegninger eller figurer (modeller) kan gi oss informasjon om størrelsene til virkelige gjenstander. Tallet som gir oss denne informasjonen kalles \outl{målestokken}.\regv
...
\[ 12\enh{m}=1\,200\enh{cm} \]
Nå har vi at
\[ \text{målestokk}=\frac{6\enh{cm}}{1\,200\enh{cm}}= \frac{6}{1\,200} \]
Vi bør også forkorte brøken så mye som mulig:
\[ \text{målestokk}=\frac{1}{200} \]
}

\info{Tips}{
Målestokk på kart er omtrent alltid gitt som en brøk med teller lik $ 1 $. I så fall kan man kan lage seg disse reglene:
\alg{
...
\eks[2]{
	...
	\abc{
		\item Vi har at
		\alg{
			\text{lengde i virkelighet}&=10,4\enh{cm}\cdot 25\,000 =260\,000\enh{cm}
		}
		Altså er det 2,6\enh{km} mellom Helland og Vike.
		
		\item Vi har at
		 \[ \text{lengde på kart}=\frac{1\,300\enh{m}}{25\,000}=0,052 \enh{m} \]
		På kartet er altså Tresfjordbrua 5,2\enh{cm}.
	}
}
\newpage
...
Noen ganger bryr vi oss ikke om \textsl{hvor mye} vi blander, så lenge forholdet er rett. For eksempel kan vi blande to fulle bøtter med solbærsirup med fem fulle bøtter vann, og fortsatt være sikre på at forholdet er riktig, selv om vi ikke vet hvor mange liter bøtta rommer. Når vi bare bryr oss om forholdet, bruker vi ordet \textit{del}. ''2 + 5'' på sirupflasken leser vi da som ''2 deler sirup på 5 deler vann''. Dette betyr at saften vår i alt inneholder $ {2+5}=7 $ deler:\vspace{3pt}
\fig{forh}
...
\eks[1]{
	Ei kanne som rommer 21\,dL er fylt med en saft der sirup og vann er blandet i forholdet $ {2:5} $.
	\abc{
	\item Hvor mye vann er det i kanna?\os
	\item Hvor mye sirup er det i kanna?
	}
	
	
	\sv
	\abc{
	\item Til sammen består saften av $ {2+5=7} $ deler. Da 5 av disse er vann, må vi ha at
	\alg{
		\text{mengde vann}&=\frac{5}{7}\text{ av 21\,dL} \br
		&= \frac{5\cdot21}{7}\enh{dL}\br
		&= 15\enh{dL}
	}
	\item Vi kan løse denne oppgaven på samme måte som oppgave a), men det er raskere å merke seg at hvis vi har 15\,dL vann av i alt 21\,dL, må vi ha $ (21-15)\enh{dL}=6\enh{dL} $ sirup.
	}
...
\eks[2]{
	...
	Når vi har 7 deler rødmaling, men ønsker 11, må vi tilsette 4 deler til. Da trenger vi
...
\eks[3]{I en ferdig blandet saft er forholdet mellom sirup og vann $ {3:5} $.\os
	...
	\sv
	Brøken vi ønsker, $ \frac{1}{4} $, kan vi skrive om til en brøk med samme teller som brøken i forholdet (altså $ \frac{3}{5} $):
