...
\op{odpar} \vs
\abc{
\item Lag en rekursiv og en eksplisitt formel for positivt partall nummer $i$. (Ta utgangspunkt i at 2 er det første partallet).
\item Lag en rekursiv og en eksplisitt formel for positivt oddetall nummer $i$. 
}
...
\op{opgfolfibn}
Figuren viser de fem første elementene i en \outl{Fibonacci-følge}. I denne følgen er $a_i>a_{i-1}$ for $i\geq 3$. De to minste kvadratene har sidelengde 1.
\abc{
\item Finn verdien til de fem første elemenentene.
\item Finn en rekursiv formel for Fibonacci-følgen (for $i\geq3$).
\item Vi kan inkludere et element $a_0$ i følgen, slik at den rekursive formelen er gyldig for $i\geq2$. Finn verdien til $a_0$.
}
\fig{opgfolfibn}
...
Gitt rekken 
\[ 8+11+14+\ldots \]
Finn $ n $ når summen av rekken er 435?
...
Forklar hvorfor den eksplisitte formelen til en aritmetisk rekke med $n$ elementer også kan skrives som
\[ a_i=a_n-(n-i)d \]

\gruboprecc{opgfolrekurs}
En følge er gitt ved at $a_1=0$ og den rekursive formelen
\[ a_i=a_{i-1}+2(i-1) \] 
Vis at den eksplisitte formelen er $a_i=i(i-1)$.

\grubeksoprecc{r2v25d1opg3}{R2V25}
En følge er gitt ved at $a_1=2$ og den rekursive formelen
\[ a_{i+1}=a_i+i+2 \]
Vis ved induksjon at den eksplisitte formelen er
\[ a_i=\frac{i(i+3)}{2} \]

\gruboprecc{div3}
Vis ved induksjon at $ n(n^2+2) $ er delelig med 3 for alle $ n\in\mathbb{N} $.

\gruboprecc{opgfolindfactorial}
Vis ved induksjon at for alle $n\in\mathbb{N}$ er\footnote{Definisjon: For et heltall $p$ er $p!=1\cdot2\cdot3\cdot...\cdot(p-1)\cdot p$}
...
\newpage
\grubop{opgfolquadar} \vso
\abc{
\item Gitt følgen $\{a_i\}$ gitt ved $1, 6, 11, 16, ...$\,. Vis at for alle $n\in\mathbb{N}$ er $(a_n)^2\in \{a_i\}$.
\item Gitt følgen ${b_i}$ gitt ved $b_i=1+d(i-1)$, hvor $d$ er et heltall. Vis at for alle $n\in\mathbb{N}$ er $(b_n)^2\in\{b_i\}$.
}
