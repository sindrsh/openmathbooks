...
\begin{equation*}
	2, 4, 8, 16 
\end{equation*}
Denne følgen kan vi skrive som $\{a_i\}$, hvor
...
\eks{
Gitt følgen\footnote{Symbolet \sym{$ ... $} viser at nye elementer fortsetter i det uendelige.}
\begin{equation*}
	2, 4, 8, 16, 32, 64, ...  
\end{equation*}
\abc{
...
\reg[Aritmetisk følge]{ \label{arfol}
...
\reg[Geometrisk følge]{ \label{geofol}
...
\eks{ \label{arrekeks}
	...
	\[ a_i=3+4(i-1) \]
	Dette er en aritmetisk rekke, og da er
	\[ a_{10} = 3+4(10-1) = 39 \]
	Altså har vi at
	\[ S_{10} = 10\cdot\frac{3+39}{2} = 210 \]
} \vsk
\fork{\ref{sumarrek} \sumarrek}{
	Ved å bruke den eksplisitte formelen fra \rref{arfol} kan vi skrive summen av en aritmetisk rekke med $ n $ elementer som
	\begin{equation}
		S_n = a_1 + (a_1+d) + (a_1+ 2d)+...+ (a_1+d(n-1)) \label{a1}
	\end{equation}
	Men et ledd i rekken kan også uttrykkes slik:
	\[ a_i= a_n-(n-i)d \]
	Og da kan vi skrive summen som (her står ledd nummer $n$ først, deretter ledd nummer $n-1$ og så videre)
	\begin{equation}
