...
Gitt en funksjon $ f(x) $, som vist i \fref{fartvar}. Vi ønsker å finne  en tilmærming for arealet $A$ avgrenset av grafen til $f$, $x$-aksen og linjene $x=a$ og $x=b$. $ I $ til $ f $ på intervallet $ [a, b] $.
...
\end{align}
hvor $ x_i=a+(i-1)\Delta x$ og $ \Delta x=\frac{b-a}{n} $ (legg merke til at $ x_1=a $).\vsk

Vi har her tatt utgangspunkt i at $f$ er positiv for alle $x\in[a, b]$. I så fall kan det vises at \sym{$\approx$} i \eqref{intdefpreeq} kan erstattes med \sym{$=$}. Hvis derimot $f$ var negativ for alle $x\in[a, b]$, måtte vi i tillegg ha multiplisert $A$ med $-1$ (forklar for deg selv hvorfor) for at likheten skulle vært gyldig. Og hvis $f$ var skiftvis positiv og negativ på intervallet, måtte vi gjort enda flere justeringer for å oppnå en gydlig likhet. Dette motiverer oss til å definere grensesummen som noe helt eget:
