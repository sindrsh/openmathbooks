...
Gitt en funksjon $ f(x) $, som vist i \fref{fartvar}. Vi ønsker å finne  en tilmærming for arealet $A$ til området avgrenset av grafen til $f$, $x$-aksen og linjene $x=a$ og $x=b$. $ I $ til $ f $ på intervallet $ [a, b] $.
\figc{int1a}{Arealet $A$ avgrenset av grafen til $f$, $x$-aksen og linjene $x=a$ og $x=b$. \label{fartvar}}
Vi starter med å dele $ [a, b] $ inn i tre delintervaller, som da får bredden $ \Delta x=\frac{b-a}{3} $. Videre bruker vi verdien til $ f(x) $ i starten av hvert delintervall som de respektive høydene i tre rektangler. $ x $-verdiene dette gjelder kaller vi $ {x_1=a} $, $ x_2 $ og $ x_3 $. Vi kan nå tilnærme $ A $ som summen av tre areal, $ s_1 $, $ s_2 $ og $ s_3 $:
\alg{
	A &\approx s_1 + s_2 + s_3 \\
...
\begin{align}
	A&\approx\lim\limits_{n\to \infty}\left[f(x_1)+f(x_2)+...+f(x_{n})\right]\Delta x \nonumber \\
	&\approx \lim\limits_{n\to \infty}\sum\limits_{i=1}^{n} f(x_{i})\Delta x \label{intdefpreeq}
\end{align}
hvor $ x_i=a+(i-1)\Delta x$ og $ \Delta x=\frac{b-a}{n} $.\vsk

Vi har her tatt utgangspunkt i at $f$ er positiv for alle $x\in[a, b]$. I så fall kan det vises at \sym{$\approx$} i \eqref{intdefpreeq} kan erstattes med \sym{$=$}. Hvis derimot $f$ var negativ for alle $x\in[a, b]$, måtte vi i tillegg ha multiplisert $A$ med $-1$ (forklar for deg selv hvorfor) for at likheten skulle vært gyldig. Og hvis $f$ var skiftvis positiv og negativ på intervallet, måtte vi gjort enda flere justeringer for å oppnå en gyldig likhet. Dette motiverer oss til å definere grensesummen som noe helt eget:
...
\newpage
I kommende seksjoner skal vi finne integraler på en helt annen måte enn i eksempelet på side \pageref{eksfinnint}. Læresetningen som sørger for dette er så viktig at den rett og slett kalles \outl{analysens fundamentalteorem}\footnote{Analyse i matematisk sammenheng kan, kort oppsummert, sies å være studien av funksjoner. Et teorem er en læresetning som kan bevises.}. Da teoremet gir oss en metode som omgår utregning av summer, lønner det seg å skrive integralet på en mer kompakt form:\regv
...
Vi har hittil sett på det \outl{bestemte integralet}, som har sitt navn fordi integralet er over et intervall der start- og sluttverdien er gitt. Det \outl{ubestemte} integralet til en funksjon $ f(x) $ skriver vi derimot som
\[ \int\limits_c^x f(t)\, \textrm{d}t \]
Navnet ubestemt kommer av at $ c $ er en vilkårlig konstant og at $ x $ er en variabel\footnote{Det kan kanskje se litt rart ut at vi har skrevet $ f(t) $ i integralet når vi snakker om $ f(x) $, men dette gjøres bare for å skille mellom variablene $ x $ og $ t $. $ x $ kan ha en hvilken som helst verdi, men for det ubestemte integralet ser vi på $ f $ for verdiene $ {t\in [a, x]} $, altså $ f(t) $. Og da er det ikke $ x $ som varierer, men $ t $, derav $ \textrm{d}t $.}.\vsk

Hvis vi lar $ F$ være en antiderivert til $ f $, har vi fra (\ref{anfund2}) at:
\[ \int\limits_c^x f(t)\, \textrm{d}t = F(x)-F(c)  \]
...
\reg[Ubestemt integral]{
	...
	\begin{equation}
		\int f(x)\deltax  = F(x)+ C \label{uint}
	\end{equation}
...
\reg[Ubestemte integraler]{
	...
	\begin{align}
		...
		& \int \sin (kx) \, dx = -\frac{1}{k}\cos (kx) + C \label{intsinkx}\\
		& \int \cos (kx) \, dx = \frac{1}{k}\sin (kx) + C \label{intcoskx}\\
		& \int e^{kx} \, dx = \frac{1}{k}e^{kx} + C \label{intexpkx}\\
...
Arealet avgrenset av grafen til $ f $, $ x $-aksen, og linjene $ {x=a} $ og $ {x=b} $ skal vi for enkelhets skyld kalle \outl{arealet avgrenset av $ f $ for $x\in [a, b] $}. I \refsec{BestemtogubestemtI} har vi sett at det er en sterk sammenheng mellom dette arealet og det bestemte integralet av $ f $:\regv
...
&= -\bigg[\frac{2}{\pi}\cos\left(\frac{\pi}{2}\cdot1\right)+1^2-1 \\ &\qquad\qquad-\left(\frac{2}{\pi}\cos\left(\frac{\pi}{2}\cdot0\right)+0^2-0\right)\bigg]\\
&= \frac{2}{\pi}
} \vs
\alg{
\int\limits_{1}^{2} (g-f)\,dx&=\left[x^2-x+\frac{2}{\pi}\cos\left(\frac{\pi}{2}x\right)\right]_1^2\\
&=2^2-2+\frac{2}{\pi}\cos\left(\frac{\pi}{2}\cdot2\right) \\ &\qquad\qquad-\left(1^2-1+\frac{2}{\pi}\cos\left(\frac{\pi}{2}\cdot1\right)\right) \\
&=2 -\frac{2}{\pi}
}
...
\newpage
Følgelig er $ \lim\limits_{n\to\infty} \sum\limits_{i=1}^{n} c_i =0,$ og da er
\begin{align}
