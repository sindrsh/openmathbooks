...
\section{Vektorfunksjoner (Parameteriseringer)}
...
\reg[\linrom \label{linrom}]{
	En linje $\vec{l}$ som går gjennom punktet $ {A=(x_0, y_0, z_0)} $ og har retningsvektor $ {\vec{r}=[a, b, c] }$ er gitt ved
	\[ \vec{l}(t)=A+t\vec{r}=[x_0+at, y_0+bt, z_0+ct] \]
	\fig{lin}
}
\eks{\label{lirep}
	En linje går gjennom punktene ${ A=(-2, 2, 1)} $ og $ {B=(2, 4, -5)} $.\os
	\abc{
	\item Finn en parameterisering for linja $ \vec{l} $ som går gjennom $ A $ og $ B $. \os
	...
	\[\vec{l}(t)= A+t\vv{r}=[-2 + 2t,	2 + t, 1 -3t] \]
	Alternativt kan vi skrive
	\[ \vec{l}(t):\left\{\begin{array}{lll}
		x=-2+2t \\
		y=2+t \\
		z=1-3t
	\end{array}\right. \]
...
\eks[3]{\label{plpare3}
Et plan $ \alpha $ gitt ved ligningen ${2x-3y-3z-11=0}  $ møter en linje $ \vec{l} $ i et punkt $ A $. Finn koordinatene til $ A $ når $ \vec{l} $ er gitt ved
		\[\vec{l}(t): \left\lbrace{\begin{array}{l}
	x= -1 -2t   \\
	y= -1 + t    \\
	z= 1 + 2t 
	\end{array} }\right. \]\vs
...
Gitt en linje $ \vec{l} $ i rommet, beskrevet av punktet \textit{A} og en retningsvektor $ \vec{r} $. I tillegg ligger et punkt \textit{B} utenfor linja.

\figc{plin}{Punkt $ B $ en avstand $ h $ fra linja $ l $. \label{plin}}
Vi ønsker nå å finne den korteste avstanden fra \textit{B} til linja. Denne avstanden identifiserer vi som høyden \textit{h} i trekanten utspent av $ \vec{r} $ og $ \vv{AB} $. Av (\ref{vektre}) har vi at arealet til trekanten er gitt ved uttrykket
