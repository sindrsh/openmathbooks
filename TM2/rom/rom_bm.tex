...
\reg[\linrom \label{linrom}]{
	En linje $\vec{l}(t)$ som går gjennom punktet $ {A=(x_0, y_0, z_0)} $ og har retningsvektor $ {\vec{r}=[a, b, c] }$ er gitt ved
...
\eks{\label{lirep}
	...
	\textbf{a)} Finn en parameterisering for linja $ \vec{l} $ som går gjennom $ A $ og $ B $. \os
...
\reg[Plan i rommet]{
	...
	\[ \alpha(s, t)=A + s\vec{u}+t\vec{v}=[(x_0 + a_1s + a_2t, y_0+b_1s+b_2t, z_0+c_1s+c_2t)] \]
}
\eks[1]{ \label{plpare}
Et plan $\alpha(s, t)$ inneholder punktene $ {A=(-2, 3, 5) }$, $ {B=(-10, 1, 9)} $ og $C= {(0, 5, -4)} $.\os
...
\textbf{b)} Vi starter med å finne en $ s $ og en $ t $ som oppfyller kravet for $ x $ og $ y $-komponentene, og får da ligningssystemet
\alg{
...
\eks[2]{\label{plroe2}
	...
	\sv
	\textbf{a)} For å finne en parameterisering til et plan gitt av en ligning, står vi fritt til selv å velge to av $ {x, y} $ og $ z $ som lik hver av parameteriseringsvariablene. Vi velger her $ {x=s} $ og $ {z=t} $, og får at \vs
	\alg{3s-y-2t+6&=0 \\
		y&= 3s+2t-6}
...
Gitt ei kule med sentrum $ {S=(x_0, y_0, z_0)} $ og et vilkårlig punkt \\${A= (x, y, z)} $, som ligger på kulen. 
