...
r = 1;
draw(-1.5*f(a) -- 1.5*f(a), L=Label("$l$", position=Relative(1)));
draw(-1.3*f(b) -- 1.5*f(b), L=Label("$m$", position=Relative(1)));
draw(o -- f(b+pi), red, arrow=Arrow, L=Label("$\vec r_1$",align=SE, position=Relative(0.7)));
draw(o -- f(a), blue, arrow=Arrow, L=Label("$\vec r_2$",position=Relative(0.7)));
