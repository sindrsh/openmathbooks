...
\subsection*{Oppgave 2}
\abc{
\item Det ytre pentagonet i figur $ n $ har $ 5(n-1) $ kuler. Da det ytre pentagonet er det eneste som skiller figur $ n $ og figur $ n-1 $, har vi at
\[ P_{n}=5(n-1)+P_{n-1} \]

\item\phantom{a} \pythonut{opg2.py}{24751}
\item 
\textbf{Løsning 1}\\
Vi har at
\alg{
...
\begin{equation}\label{opg2i}
	P_n=1+\frac{5n(n-1)}{2}
\end{equation}
Det er åpenbart at formelen stemmer for $ n=1 $. Vi antar at den eksplisitte formelen gjelder for $ P_n $, da kan vi skrive
\begin{align}
	P_{n+1}&=P_{n+1}=5(n+1-1)+P_n \nonumber\\
	&=5n+P_n \nonumber \\
	&= 5n+1+\frac{5n(n-1)}{2} \nonumber \\
...
\end{align}
\eqref{opg2i} samsvarer med \eqref{opg2ii}, og dermed er induksjonsbeviset fullført. \\[12pt]
\textbf{Løsning 2} \\
Bruker regresjon, og finner $ P(n)=P_n $ i celle 1. Sjekker at $ P(n) $ er sann for $ P(n) $ (celle 2). Antar at kravet fra rekursiv formel er oppfylt for $ P(n) $, og sjekker at det da er oppfylt for $ P(n+1) $. Begge sjekkene stemmer, og dermed er induksjonsbeviset fullført.
\begin{figure}
	\centering
	\includegraphics[scale=0.2]{opg2c}
\end{figure}
}
\subsection*{Oppgave 3}
...
\begin{figure}
	\centering
	\includegraphics[scale=0.3]{opg5a}
\end{figure}
\item Vi definerer $ r_2(t) $ (celle 7), og finner en normalvektor til smygplanet (celle 8). I celle 9-13 finner vi cosinusverdien til vinkelen mellom smygplanet og $ y $-aksen, $ x $-aksen og $ z $-aksen. Vi ser da at smygplanet har konstant vinkel mellom alle aksene, og at smygplanet er parallell med $ y $-aksen, fordi cosinusverdien er 0 (celle 9).
}
\begin{figure}
	\centering
	\includegraphics[scale=0.3]{opg5d}
\end{figure}
