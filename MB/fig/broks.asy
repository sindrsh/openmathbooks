...
void mkbroktick(int tc,real b, int j=0, int i=0, real y=0, real xs=0, pen p=black){
	real dx = 1/tc;
	draw((xs,y)--(xs+b,y));
	for (i; i*dx<= b; ++i){
			real x = i*dx;
...
void brbr(int m, int n, real y=0.1, bool l=true, real xs=0,pen p=blue, bool arw=false){
	...
	if (arw){
		draw(shift(xs,0)*s, p, arrow=Arrow(DefaultHead), L=Label(lb, position=Relative(0.5),1.5N));
	}
	else{
		draw(shift(xs,0)*s, p, L=Label(lb, position=Relative(0.5),1.5N));
		mktc(xs+m/n, p, y=y);
		}
	mktc(xs+0, p, y=y);	
}
