...
\fig{punkt1}
\textbf{Linjer og linjestykker}\os
En rett strek som er uendelig lang (!) kaller vi en \outl{linje}\index{linje}. At linja er uendelig lang, gjør at vi aldri kan \textsl{tegne} en linje, vi kan bare \textsl{tenke} oss en linje. Å tenke seg en linje kan man gjøre ved å lage en rett strek, og så forestille seg at endene til streken vandrer ut i hver sin retning.
\fig{linj1}
En rett strek som går mellom to punkt kaller vi et \outl{linjestykke}\index{linjestykke}.
\fig{linjstk1}
Linjestykket mellom punktene $ A $ og $ B $ skriver vi som $ AB $. \outl{Lengden} til $ AB $ er lengden vi må vandre langs linjestykket for å gå fra $ A $ til $ B $. \vsk

 I en figur viser vi gjerne lengden til et linjestykke med et tall midt på linjestykket. I figuren under vises det at lengden til $AB$ er 3.
\fig{linjstk1_withlength}
...
\begin{itemize}
\item Til hvilken tid vi snakker om et linjestykke og hvilken tid vi snakker om en lengde vil komme tydelig fram av sammenhengen begrepet blir brukt i.
\item Å hele tiden måtte ha skrevet ''lengden til $ AB $'' og\\ lignende ville gitt mindre leservennlige setninger.
\end{itemize}
}
\subsubsection{Buer}
En strek med en hvilken som helst form kalles en \outl{bue}\index{bue} eller en \outl{kurve}\index{kurve}.
\fig{curve}
En bue uten endepunkter kalles en \outl{lukket bue}\index{bue!lukket}. En lukket bue danner én eller flere lukkede former.
\fig{curveclosed}
\newpage
\textbf{Avstand}\os
Det er et uendelig antall veier man kan gå fra ett punkt til et annet, og noen veier vil være lengre enn andre. Når vi snakker om avstand i \\geometri, mener vi helst den \textsl{korteste} avstanden. For geometrier vi skal ha om i denne boka, vil den korteste avstanden mellom to punkt alltid være lengden til linjestykket (blått i figuren under) som går mellom punktene.
\fig{linjstk2}
\textbf{Sirkler; sentrum, radius og diameter} \os
...
Når vi skal måle en vinkel i grader, tenker vi oss at en sirkel er delt inn i 360 like lange buer. Én slik bue kaller vi én \outl{grad}\index{grad}, som vi skriver som tegnet \sym{$ ^\circ $}. Vinkelstørrelsen måles i antall grader det er mellom to linjestykker som har endepunkter på sirkelen og i sirkelens sentrum.
\fig{vink3}
\newpage
Legg merke til at en $ 90^\circ $ vinkel markeres med symbolet \sym{$ \square $}. En vinkel som måler $ 90^\circ $ kalles en \outl{rett  vinkel}\index{vinkel!rett}. Linjer som danner rette vinkler sier vi står \outl{vinkelrette}\index{vinkelrett} på hverandre. Dette viser vi med symbolet $ \sym{$ \perp $} $.
\[ AB\perp CD \]
\fig{vink3a}
...
\reg[\corra \label{corra}\index{vinkel!samsvarende}]{
Vinkler med forskjellige toppunkt og et høyre vinkelbein felles eller et venstre vinkelbein felles, kalles \outl{samsvarende vinkler}. \os

Samsvarende vinkler med parallelle vinkelbein er like store.
}
\newpage
\eks[1]{
I figuren under er $\angle BGC$, $\angle AGD$ og $\angle FHC$ samsvarende vinkler fordi alle har $DC$ som venstre vinkelbein. I tillegg er $\angle BGC=\angle AGD$.
\fig{corraa}
}
\eks[2]{
I figuren under er ${\angle BGC=\angle AGD=\angle FHC}$ fordi de er samsvarende vinkler med parvis parallelle vinkelbein.
\fig{corrab}
}
\fork{(\ref{corra}) \corra}{
Gitt tre samsvarende vinkler $\angle BGC$, $\angle AGD$ og $\angle FHC$ med parvis parallelle vinkelbein, som vist i figuren under.
\fig{corrab}
At ${\angle FHC=\angle BGC}$ godtar vi som sant uten noen videre\\ forklaring. ${\angle BGC=\angle AGD}$ fordi de er toppvinkler, og dermed er
\[ \angle BGC=\angle AGD=\angle FHC \]
}
...
\fig{kant1}
Linjestykkene en mangekant består av kalles \outl{kanter}\index{kant} eller \outl{sider}\index{side!i mangekant}. Punkt-ene der kantene møtes kaller vi \outl{hjørner}\index{mangekant!hjørner i}. Trekanten under har altså hjørnene $ A $, $ B $ og $ C $, og sidene (kantene) $ AB $, $ BC $ og $ AC $.
\fig{kant2}
En \outl{regulær mangekant} er en mangekant hvor alle sidene er like lange.
\fig{ngon}
\info{Merk}{
...
\textbf{Diagonaler} \os
Et linjestykke mellom to hjørner som ikke hører til samme side i en mangekant kalles en \outl{diagonal}. I figuren under ser vi \\diagonalene $ AC $ og $ BD $.
\fig{kant7}
\newpage
\subsubsection{Høyde og grunnlinje \label{grunnlinje}}
Når vi i \hrs{Areal}{seksjon} skal finne areal, vil begrepene \textit{grunnlinje}\index{grunnlinje} og \textit{høyde}\index{høyde} være viktige. For å finne en høyde i en trekant, tar vi utgangspunkt i én av sidene. Siden vi velger kaller vi \outl{grunnlinja}. La oss starte med $ AB $ i figuren under som grunnlinje. Da er \outl{høyden} linjestykket som går fra $ AB $ (eventuelt, som her, forlengelsen av $ AB $) til $ C $, og som står vinkelrett på $ AB $.
\fig{tri15}
Da det er tre sider vi kan velge som grunnlinje, har en trekant tre høyder.
\fig{tri15b}
\info{Merk}{Høyde og grunnlinje kan også på lignende vis bli brukt i forbindelse med andre mangekanter.}
\section{Egenskaper for trekanter og firkanter}
I tillegg til å ha et bestemt antall sider og hjørner, kan mangekanter også ha andre egenskaper, som for eksempel sider eller vinkler av lik størrelse, eller sider som er parallelle. Vi har egne navn på \text{mangekanter} med spesielle egenskaper, og disse kan vi sette opp i en oversikt der noen ''arver''\footnote{I \rref{trekantar} og \rref{firkantar} er dette indikert med piler.} egenskaper fra andre.\regv
...
\fork{\ref{180} \trisum}{
	\fig{geo10}	
	Vi tegner et linjestykke $ FG $ som går gjennom $ C $ og som er parallell med $ AB $. Videre setter vi punktet $ E $ og $ D $ på forlengelsen av henholdsvis $ AC $ og $ BC $. Da er $ {\angle BAC=\angle GCE} $ og $ {\angle CBA=\angle DCF} $. $ {\angle ACB=\angle ECD}  $ fordi de er toppvinkler. Vi \\har at
	\[ \angle DCF+\angle ECD+\angle GCE=180^\circ \]
	Altså er
	\[ \angle CBA+\angle ACB+\angle BAC=180^\circ  \]
} 
\reg[\grside \label{grside}]{
Hvis $AC>BC$, er $\angle B>\angle A$. \os

Hvis $\angle B>\angle A$, er $AC>BC$.

\fig{grside}
}
\fork{(\ref{grside})\ \grside}{
Forklaringen for denne regelen finner du i løsningsforslaget til \refgrub{opggeotrekulik}.
}

\spr{
	Den lengste siden i en rettvinklet trekant kalles \outl{hypotenus}\index{hypotenus}. De korteste sidene kalles \outl{kateter}\index{katet}.
}
...
\section{\omkr}
Når vi måler hvor langt det er rundt en lukket form, finner vi\\ \outl{omkretsen}\index{omkrets} til formen. La oss finne omkretsen til dette rektangelet:
\fig{geo1}
Det første vi må gjøre er å definere et linjestykke med lengde 1:
\fig{lengde1}
Rektangelet har to sider med lengde 4 og to sider med lengde 5:
\fig{geo1a}
Dette betyr at
\[ \text{Omkretsen til rektangelet} = 4+4+5+5 = 18 \]
\reg[Omkrets]{Omkretsen er lengden rundt en lukket figur.}
\eks[]{ \vsb \vs \vs
	\begin{figure}
		\centering
...
Når vi ønsker å si noe om hvor store overflaten til en form er, finner vi \outl{arealet}\index{areal} til formen. Grunnlaget for å bestemme arealet til en form er dette:\regv

\st{Vi tenker oss et kvadrat med sidelengder 1. Dette kaller vi\\ 'enerkvadratet'.
	\fig{tri_10}
	Så finner vi ut hvor mange 'enerkvadrat' det er plass til på overflaten til formen.}
\subsubsection{\arrekt \label{arrekt}}
La oss finne arealet til et rektangel med grunnlinje 3 og høyde 2.
\fig{tri11}
...
\[ \text{Arealet til rektangelet} = 3\cdot 2 = 6  \]
\newpage
\reg[\arrekt \label{arfir}]{
	\[ \text{areal}=\text{grunnlinje}\cdot\text{høyde} \]
	\fig{tri12}
}
\spr{Ofte blir ordene \outl{bredde}\index{bredde} og \outl{lengde}\index{lengde} brukt om sidelengdene til et rektangel.}
\info{Merk}{
Lengdene på figurer vil ikke nødvendigvis samsvare. En sidelengde lik 1 på én figur kan altså være kortere enn en sidelengde lik 1 på en annen figur.
}
\eks[1]{
...
\textit{1) Tilfellet der grunnlinja og høyden har et felles endepunkt} \os
La oss finne arealet til en rettvinklet trekant med grunnlinje $ 5 $ og høyde $ 3 $.
\fig{tri16}
...
\[\frac{5\cdot3}{2}= \frac{\text{grunnlinje}\cdot\text{høyde}}{2} \]
\newpage
\textit{2) Tilfellet der høyden ligger inni trekanten, men ikke har felles\\ endepunkt med grunnlinja} \os
Trekanten under har grunnlinje 5 og høyde 4.
\fig{tri20}
Med denne trekanten (og høyden) som utgangspunkt, danner vi denne figuren:
\fig{tri20a}
...
\end{itemize}
Summen av arealene til den gule og den røde trekanten utgjør altså halvparten av arealet til rektangelet som består av alle de fire fargede trekantene. Arealet til dette rektangelet er $ 5\cdot4 $, og da vår opprinnelige trekant (den blå) består av den røde og den gule trekanten, har vi at
\[ \text{Arealet til den blå trekanten}=\frac{5\cdot4}{2}=\frac{\text{grunnlinje}\cdot\text{høyde}}{2} \] 
\newpage
\textit{3) Tilfellet der høyden ligger utenfor trekanten} \os
Trekanten under har grunnlinje 4 og høyde 3. 
\fig{tri18}
...
\[ \frac{4\cdot3}{2}=\frac{\text{grunnlinje}\cdot\text{høyde}}{2} \]
\newpage
...
\reg[Arealet til en trekant]{
	\[ \text{Areal}=\frac{\text{grunnlinje}\cdot\text{høyde}}{2} \]
\fig{triar00}
}
\eks[1]{
...
\fig{3db}
Mens et rektangel sies å ha en bredde og en høyde, kan vi si at boksen over har en bredde, en høyde \textsl{og} en lengde (dybde). Overflaten til \\boksen i figuren over består av 6 rektangler. Mangekanter som er deler av en overflate kalles \outl{sideflater}.
...
\reg[Tredimensjonale figurer]{
	...
	\parbox[r][][l]{0.6\linewidth}{
		...
		Har tre par med like rektangler som sideflater. Alle sideflatene som er i kontakt, står vinkelrette på hverandre.
	}
	\parbox[l][][l]{0.4\linewidth}{
		\centering
		\fig{kube}	
	...
	\parbox[r][][l]{0.6\linewidth}{
		...
		To av sideflatene er like trekanter som er parallelle. Har tre sideflater som er rektangler.
	}
	\parbox[l][][l]{0.4\linewidth}{
		\centering
		\fig{fpyr}	
	...
	\parbox[r][][l]{0.6\linewidth}{
		\textbf{Kjegle}\\
		En del av overflaten er en sirkel, den resterende delen er en sammenføyd sektor.
	}
	\parbox[l][][l]{0.4\linewidth}{
	\centering
	\fig{kule}	
...
\newpage
\info{Tips}{Det er ikke så lett å se for seg hva en \textit{sammenføyd sektor} er, men prøv dette: \vsk
...
\fig{vol1}
Ei slik kube kan vi kalle ''enerkuben''. Si vi har et firkantet prisme med bredde 3, lengde 4 og høyde 2.
\fig{vol2}
I denne er det plass til akkurat 24 enerkuber.
\fig{vol2a}
Dette kunne vi ha regnet slik:
\[ 3\cdot 4\cdot 2=24\]
\reg[Volumet til et firkantet prisme I]{
	\fig{vol2c}
	\[\text{volum}=\text{bredde}\cdot\text{lengde}\cdot\text{høyde} \]
}
\newpage
\subsubsection{Grunnflate}
For å regne ut volumet til de mest elementære formene vi har, er det lurt å bruke begrepet \outl{grunnflate}\index{grunnflate}. Slik som for en grunnlinje\footnote{Se side \pageref{grunnlinje}.}, er det vårt valg av grunnflate som avgjør hva som er høyden. For prismet fra forrige side er det naturlig å velge den flaten som ligger horisontalt til å være grunnflata. Arealet til grunnflaten skriver man gjerne som $ G $:
\fig{vol2b}
Grunnflaten har areal $ 3\cdot4=12 $, mens høyden er 2. Volumet til hele prismet er grunnflaten sitt areal ganget med høyden:
\alg{
	\text{volum} &= \colb{3\cdot 4} \cdot 2 \\
	&= \colb{G}\cdot 2\\
	&= 24
}
\reg[Volumet til et firkantet prisme II]{
	\fig{vol2d}
	\[ \text{volum}=G\cdot \text{høyde} \]
}


\section{Symmetri}
Mange figurer kan deles inn i minst to deler hvor den ene delen bare er en forskjøvet, speilvendt eller rotert utgave av den andre. Dette kalles \outl{symmetri}\index{symmetri}. De tre kommende regelboksene definerer de tre\\ variantene for symmetri, men merk dette: Symmetri blir som regel\\ intuitivt forstått ved å studere figurer, men er omstendelig å beskrive med ord. Her vil det derfor, for mange, være en fordel å hoppe rett til eksemplene.
...
\eks[]{
	Figuren under viser $ \triangle ABC $ og en blå vektor.
	\fig{trans1a}
	Figuren under viser $ \triangle ABC $ forskjøvet med den blå vektoren. 
	\fig{trans1}
}
...
\eks[1]{
	$A$, $B$, $C$ og $D$ er midtpunktene til sidene. Den røde linja og den blå linja er begge symmetrilinjer til det grønne rektangelet.
	\fig{sym2}
}
\eks[2]{
	Under vises en form laget av punktene $ A, B, C, D $ og $ E $, og denne formen speilet om den blå linja.
	\fig{sym3}
}
