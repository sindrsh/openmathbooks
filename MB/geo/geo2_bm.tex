...
\begin{document}
\section{Formler for areal, omkrets og volum}
I \refsec{Areal}\ har vi allerede sett på formler for arealet til rektangel og trekanter, men der brukte vi ord i stedet for symboler. Her skal vi gjengi disse to formlene i en mer algebraisk form, etterfulgt av andre klassiske formler for areal, omkrets og volum.\regv
...
\alg{
A_1 &=\frac{4b\cdot3}{2}=6b \vn
A_2 &=\frac{3b\cdot 2}{2}=3b\vn
A_3&=\frac{2b\cdot5}{2}=5b
}	
...
\info{Merk}{
	Når man tar utgangspunkt i en grunnlinje og en høyde, er areal-formlene for et parallellogram og et rektangel identiske. Å anvende \rref{artrap} på et parallellogram vil resultere i et uttrykk tilsvarende $ gh $. Dette fordi et parallellogram er et spesialtilfelle av et trapes (og et rektangel er et spesialtilfelle av et parallellogram).
}
\info{Tips}{
	...
	\[A=\frac{1}{2}\cdot g h= \frac{1}{2}\cdot\frac{3}{4}\cdot\frac{7}{9}=\frac{21}{72} \]
}
\newpage
\spr{\index{$\pi$}
Det irrasjonale tallet 3,141592653589793... er så viktig i mate-matikk at det har sitt eget symbol, nemlig \sym{$\pi$}. Altså er\footnote{Se forklaringen til \rref{omkrsirk} for hvordan man finner verdien til $\pi$.}
\[ \pi=3,141592653589793... \]

$\pi$ uttales ''pi''\index{pi}. 
} \vsk

\reg[\omkrsirk \label{omkrsirk}]{\index{omkrets!til sirkel}
...
På samme måte som vi på side \pageref{arsirkforkl} delte en sirkel inn i et \\''rektangel'' med sidelengder tilnærmet lik $ r $ og $ \pi r $, kan vi dele en sektor inn i et ''rektangel'' med sidelengder tilnærmet lik $ r $ og $ \frac{1}{2}a $. Arealet $ A $ til ''rektangelet'', altså sektoren, blir da
\[ A=\frac{1}{2}ar \]
} \vsk
...
hvor $ \frac{a}{2\pi r} $ uttrykker forholdet mellom buelengden til sektoren og omkretsen til en sirkel med radius $ r $. Dette forholdet er også uttrykt ved $ \frac{v}{360^\circ} $, og dermed er
$ A=\pi r^2 \frac{v}{360^\circ} $
}

\newpage
\reg[\pyt \label{pyt}]{
	I en rettvinklet trekant er arealet til kvadratet dannet av den lengste siden lik summen av arealene til kvadratene dannet av de to andre sidene.\regv
...
\fork{\ref{pyt} \pyt}{ \label{pytforklaringintro}
	...
	\begin{enumerate}
		...
		\item Om vi tar bort de to oransje rektanglene og de fire grønne trekantene, er det igjen (av pkt. 2) et like stort areal til venstre som til høyre.
		\fig{tri26e}
		Dette betyr at
...
\spr{
	Den lengste siden i en rettvinklet trekant kalles \outl{hypotenus}\index{hypotenus}. Hver av de korteste sidene kalles \outl{katet}\index{katet}. Dette spesielt i samband med Pytagoras' setning.
}
\info{Merk}{
...
\reg[\volforml \label{volforml}]{ 
	Volumet $ V $ til et prisme eller en sylinder med grunnflateareal $ G $ og høyde $ h $ er
	\[ V = G h \]
	\begin{figure}
		\centering
		\footnotesize
...
\eks[1]{\vs
	\fig{vol5}
	En sylinder har radius 7 og høyde 6. 
	\abc{
		\item Finn grunnflatearealet til sylinderen.
...
\eks[2]{ \vs
	...
	\abc{
		\item Finn grunnflatearealet til pyramiden.
...
\reg[\kongtre \label{kongtre}]{ \index{trekant!kongruent}
...
\fig{geo8}
At trekantene i figuren over er formlike skrives \[ \triangle ABC\sim \triangle DEF \]
} \vsk
\newpage
...
\begin{itemize}
	\item ${\frac{DE}{AB}=2}$ er forholdstallet mellom samsvarende sider i $\triangle DEF$ og $\triangle ABC$.
	\item $BC=3$ er den samsvarende siden til $EF$.
\end{itemize}
Likningen vi satt opp i eksempelet ender med at vi ganger forholdstallet med den samsvarende siden. Dette vil alltid være tilfellet så lenge bare én av sidene er ukjent. I stedet for å sette opp en likning, kunne vi altså direkte regnet ut at $EF=2\cdot3=6$.
}
\newpage
\info{Merk}{
...
\eks[1]{
$ \angle ACB =90^\circ $. 
Vis at $ \triangle ABC \sim \triangle ACD $.
\fig{geo15} \vs
\sv
...
\mers{
På en tilsvarende måte kan det vises at $ \triangle ABC \sim \triangle CBD$.
}
}
\newpage
\eks[2]{
...
\alg{
\frac{AC}{IG}=\frac{18}{12}=\frac{3}{2}\quad,\quad\frac{BC}{IH}=\frac{9}{6}=\frac{3}{2}\quad,\quad \frac{AB}{HG}=\frac{12}{8}=\frac{3}{2}
}
...
{
\textit{Vi skal her bruke \textit{regulære} mangekanter for å komme til ønsket resultat. Da det er utelukkende regulære mangekanter vi kommer til å bruke, vil de bli omtalt bare som mangekanter.}\vsk
...
(Ved hjelp av mer avansert matematikk kan det vises at omkretsen til en sirkel med radius 1 er et irrasjonalt tall, men at alle desimalene vist over er korrekte, derav likhetstegnet.) \vsk
\newpage
...
Forholdstallet mellom omkretsen og diameteren i en sirkel skriver vi som $ \pi $ \index{$ \pi $}:
\[ \frac{O}{D}=\pi \]
...
\fork{\ref{pytomv} \pytomv}{
	Samme hva verdien til $ a $ og $ b $ måtte være, kan man åpenbart alltid lage en rettvinklet trekant med kateter $ a $ og $ b $. Av \pyt\;er hypotenusen da $ \sqrt{a^2+b^2} $. Av vilkår (ii) i \rref{konsttre} er dette en unik trekant, og det betyr at alle trekanter med sidelengder $ a $, $ b $ og $ \sqrt{a^2+b^2} $ er rettvinklet. Og for en slik trekant er $a^2+b^2=c^2$.
	
	\vsk
	
	Si videre at trekanten \textsl{er} rettvinklet, men at $a^2+b^2\neq c^2$. Dette ville motsi Pytagoras' setning, og kan dermed ikke være tilfellet.
}
...
\newpage
\info{Merk}{
	I de kommende forklaringene av vilkårene \textsl{ii)} og \textsl{iii)} fra \rref{konsttre} tar man utgangspunkt i følgende:
	\begin{itemize}
		\item To sirkler skjærer hverandre i maksimalt to punkt.
...
\fork{\ref{konsttre} \konsttre}{
	...
	Vi lager et linjestykke $ AB $ med lengde $ c $. Så stipler vi to vinkelbein slik at $ \angle {A= u} $ og $ {\angle B=v} $. Så lenge disse vinkelbeina ikke er parallelle, må de nødvendigvis skjære hverandre i ett, og bare ett, punkt ($ C $ i figuren). Sammen med $ A $ og $ B $ vil dette punktet danne en trekant som er unikt gitt av $ c $, $ u $ og $ v $.
	\fig{geo13c}		
	\textbf{Vilkår (ii)}\os
...
\fork{\ref{vilkform} \vilkform}{
	...
	Hvis $ \angle A=\angle D $ og $ \angle B=\angle E $, følger det at $ \angle C=\angle F $.\vsk
	
	\textbf{Vilkår (ii)}\os
	Vi tar utgangspunkt i trekantene $ \triangle ABC $ og $ \triangle DEF $ der ${\angle C = \angle F}$ og
	\begin{equation}
	...
	\textbf{Vilkår (iii)}\os
	Gitt to trekanter $ \triangle ABC $ og $ \triangle DEF $ der
	\begin{equation}
		\frac{AB}{DE}=\frac{AC}{DF}=\frac{BC}{EF} \label{vilkformb}
	\end{equation}
	Vi setter ${AC=b}$, ${AB=c}$, ${EF=d}$, ${DF=e}$, ${DE=f}$. Videre plasserer vi $ D' $ og $ E' $ på henholdsvis $ AC $ og $ BC $, slik at $ {D'C=e} $ og $ {E'C=d} $. Av vilkår (i) fra \rref{vilkform} har vi da at $ {\triangle ABC\sim\triangle D'E'C} $. Altså er
	\alg{
