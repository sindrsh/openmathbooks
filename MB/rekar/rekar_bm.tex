...
\begin{document}
\newpage
\section{Matematiske operasjoner}
Hvis vi kombinerer to eller flere tall for å lage et nytt tall, utfører vi en \outl{matematisk operasjon}\index{matematisk operasjon}. \outl{Addisjon}, \outl{subtraksjon}, \outl{multi-plikasjon} og \outl{divisjon} er våre grunnleggende matematiske operasjoner, og vi kaller dem de fire grunnleggende \outl{regenartene}\index{regneart}. Når vi bruker disse regneartene, lager vi oss \outl{regnestykker}\index{regnestykke}. Verdien vi får av å utføre en matematisk operasjon kaller vi gjerne \outl{svaret} til regnestykket.

\section{Addisjon \label{Addisjon} \index{addisjon}}	
...
\fig{plusm1a}
\spr{
	Et addisjonsstykke består av to eller flere \outl{ledd}\index{ledd} og én \outl{sum}\index{sum}. I addisjonsstykket
	%id{2plus3}
	\[ 2+3=5 \]
	%\id
	...
	\end{itemize}
Det å legge sammen tall kalles også \outl{å summere}\index{summere}.
} \regv
\newpage
...
\section{Subtraksjon \label{Subtraksjon}} \index{subtraksjon}
\subsection*{Subtraksjon med mengder: Å trekke fra}
...
\info{En ny tolkning av 0}{
	I \refkap{Talavare} nevnte vi at 0 kan tolkes som ''ingenting''. Subtraksjon gir oss muligheten til å uttrykke 0 ved andre tall. For eksempel er 
	%id{7min7}
	$ {7-7=0} $ 
	%\id
	og 
...
%id{arrow}
\footnote{I figurer med tallinjer vil rødfargede piler fortelle at man starter ved pilspissen og går til den andre enden.}:
%\id
 \regv
\info{Merk}{
	I \textsl{Eksempel 1} og \textsl{Eksempel 2} under går vi i motsatt retning av den som pila peker i. Dette betyr at vi starter i pilspissen, og går til enden. Dette kan først virke litt rart, men spesielt i \refkap{Negtal} vil det lønne seg å tenke slik.
} 
\eks[1]{ \vs
	%id{6min4}
	\[ 6-4=2 \]
...
Figurene som viser addisjon og subtraksjon på tallinja, viser hvorfor vi sier at addisjon og subtraksjon er \outl{omvendte\\ operasjoner}\index{omvendt operasjon}. \vsk

Sagt med et eksempel: Siden $2+4=6$, så vet vi at $6-4=2$ og at $6-2=4$.
}

\info{Å kansellere ledd}{
Om vi har et uttrykk hvor tall med lik verdi er addert like mange ganger som de er subtrahert, sier vi at de \outl{kansellerer} hverandre. I uttrykket
%id{mincncla}
\[ 4+3-4+4+7-4+1 \]
%\id
har vi like mange 4-ere addert som vi har 4-ere subtrahert. Den samlede summen og differansen av 4-erene er lik 0. Dermed kan vi forenkle uttrykket vårt til
%id{mincnclb}
\[ 3+7+1 \]
%\id
...
\section{Ganging (multiplikasjon) \label{Gonging} } \index{ganging}\index{multiplikasjon}
...
\spr{
	...
	\sym{$ \times $}
	%\id
	som gangetegn. I de fleste programmeringsspråk brukes symbolet
	%id{times*}
	\sym{*}
	%\id
	\!.
...
%\id
La oss bruke en figur for å se for oss gangestykket 
%id{2times3}
$ 2\cdot3 $
%\id
\!\!:
...
%\id
 er ved å tenke slik:
%id{2times4lang}
\[\text{''} 2\cdot 4 \text{ betyr å gå 2 plasser \textsl{mot høyre}, 4 ganger.}\text{''} \]
%\id
%id{2times4eq8}
\[ 2\cdot4=8 \]
%\id
\fig{2t4l}
Også tallinja kan vi bruke for å se at rekkefølgen av faktorene ikke har noe å si:
%id{4times2lang}
\[\text{''} 4\cdot 2 \text{ betyr å gå 4 plasser \textsl{mot høyre}, 2 ganger.}\text{''} \]
%\id
%id{4times2eq8}
\[ 4\cdot2=8 \]
%\id
...
$ {3\cdot2} $
%\id
\!\!, men at multiplikasjon er en kommutativ operasjon (\rref{gangkom}) gjør at den ene tolkningen ikke utelukker den andre; 
...
$ {2\cdot3=3+3} $ 
%\id
er to uttrykk med samme verdi, og dermed kan vi også skrive $2\cdot3=3+3$ og $3\cdot2=2+2+2$.\regv

\reg[Ganging som gjentatt addisjon \label{ganggjad2}]{
	Ganging med et positivt heltall kan uttrykkes som gjentatt addisjon.
}
\eks[1]{\vsb
	%id{repeatedaddition}
	\alg{
...
\info{Merk}{
	At ganging med positive heltall kan uttrykkes som gjentatt\\ addisjon, utelukker ikke andre uttrykk. Det er ikke feil å skrive at 
	%id{2times3alt}
	$ {2\cdot 3=1+5} $
	%\id
	\!\!.
...
\section{Divisjon (deling) \label{Divisjon}} \index{divisjon}\index{deling}
%id{divsym}
\sym{:}
%\id
er \outl{divisjonstegnet}.
Divisjon har tre forskjellige betydninger, her vist ved 
%id{12div3}
$ 12:3 $
%\id
\!\!:\regv
...
\reg[Divisjon sine tre betydninger]{ \vs
	%id{divinterpret}
	\begin{itemize}
		\item \textbf{Lik fordeling av mengder} \\
		$ 12:3 = \text{''Antallet i hver gruppe når 12 deles inn i 3 like}$\\
		\hspace{1.6cm}store grupper'' 
		\item \textbf{Antall ganger et tall går på et annet} \\
...
\newpage
\subsection*{Lik fordeling av mengder}
Regnestykket 
%id{12div3}
$ {12:3} $
%\id
forteller oss at vi skal fordele 12 i 3 like store \\grupper:
\fig{del1}
...
\subsection*{Antall ganger et tall går på et annet}
\fig{del2}
