...
label("2",sh*(0,0));
label("6",sh*(dx,0));
label("3",sh*(2dx,0),blue);

label("$-$",sh*(-2dx,dy));
label("$=$",sh*(-2dx,2dy-0.025));

label("7",sh*(dx,dy));
label("4",sh*(2dx,dy),red);
draw(shift(-0.5dx,1.5dy)*sh*scale(3)*((0,0)--(dx,0)));

draw(sh*shift(0.8dx,-0.1dy)*p);
label("$10$",sh*(2dx,-0.75dy),blue+fontsize(6pt));

label("${\color{blue}10}+{\color{blue}3}-{\color{red}4}=\color{heavygreen}9$",sh2*(0,0),align=W,fontsize(10pt));

label("\color{heavygreen}9",sh*(2dx,2dy));
/*
