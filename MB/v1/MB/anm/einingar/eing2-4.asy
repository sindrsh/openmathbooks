...
label((-6.65,1.8),"\textit{Eksempel 2}",blue+fontsize(8pt),align=E);
...
fill(shb*tb,heavygreen+opacity(0.1));
...
label("52 km\,$=$\phantom{\,52\,000} m",(-2,-0.25),align=E);
...
label("0",sh*(7dx,dy/2));
draw(sh*shift(6dx,0)*p,black+0.2pt);
label(",",sh*(8dx,dy-0.04));
label("$0$",sh*(9dx,dy/2));	
draw(sh*shift(8dx,0)*p,black+0.2pt);
label(",",sh*(10dx,dy-0.04));
*/
