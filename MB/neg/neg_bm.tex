...
\fig{neg1}
Men hva om vi går andre veien, altså til venstre? Dette spørsmålet svarer vi på ved å innføre \textit{negative tall}. \index{tall!negativt}\index{tall!positivt}\regv

\reg[Positive og negative tall]{
På en tallinje gjelder følgende:
\begin{itemize}
	\item Tall plassert \textsl{til høyre} for 0 er \outl{positive tall}.
	\item Tall plassert \textsl{til venstre} for 0 er \outl{negative tall}.
\end{itemize}
\fig{neg2}
}\vsk 
Vi kan ikke hele tiden bruke en tallinje for å avgjøre om et tall er\\ negativt eller positivt, og derfor bruker vi et tegn for å vise at tall er negative. Dette tegnet er \sym{$ - $}, altså det samme tegnet som vi bruker ved subtraksjon. $ 5 $ er med dét et positivt tall, mens $ -5 $ er et negativt tall. På tallinja er det slik at
\begin{itemize}
	\item 5 ligger 5 enerlengder \textsl{til høyre} for 0.
	\item $ -5 $ ligger 5 enerlengder \textsl{til venstre} for 0.
\end{itemize}
\fig{neg3}
Den store forskjellen på $ 5 $ og $ -5 $ er på hvilken side av 0 tallene ligger. Da $ 5 $ og $ -5 $ har lik avstand til 0, sier vi at $ 5 $ og $ -5 $ har lik \outl{lengde}\index{lengde}. \regv
...
Lengden til et negativt tall er verdien til det positive tallet med samme siffer (og sifferplassering).
} 
\eks[1]{ \vs
\[ |27|=27 \]
}
...
\vsk
\info{Fortegn}{
\outl{Fortegn}\index{fortegn} er en samlebetegnelse for \sym{$ + $} og \sym{$ - $}. For eksempel; $ 5 $ har \sym{$ + $} som fortegn og $ -5 $ har \sym{$ - $} som fortegn.
}
\info{Negative tall i et koordinatsystem}{
\fig{kordneg}
}
\newpage
\section{\negrekn \label{rekmneg}}
Med innføringen av negative tall får de fire regneartene nye sider som vi må se på trinnvis. Når vi adderer, subtraherer, multipliserer eller dividerer med negative tall vil vi ofte, for tydeligheten sin skyld, skrive negative tall med parentes rundt. Da skriver vi for eksempel $ -4 $ som $ (-4) $. 
...
\[ 7+(-4)=\text{''}7 \text{ og like langt og i \textsl{samme} retning som $-4$''}\]
...
\[ 2-(-6)=\text{''}2 \text{ og like langt og i \textsl{motsatt} retning som $-6$''}\]
$ -6 $ har lengde 6 og retning \textsl{mot venstre}. Når vi skal gå samme lengde, men i \textsl{motsatt} retning, må vi altså gå lengden 6 \textsl{mot høyre}\footnote{Vi minner enda en gang om at rødfargen på pila viser at man skal vandre fra pilspissen til andre enden.}. Dette er det samme som å addere 6:
\[ 2-(-6)=2+6=8 \] \vsb
\fig{neg7}
...
I \hrs{Gonging}{seksjon} introduserte vi ganging med positive heltall som gjentatt addisjon. Med våre utvidede begrep av addisjon og subtraksjon kan vi også utvide begrepet multiplikasjon: \regv

\reg[Multiplikasjon med positive og negative tall\label{gongneg}]{
Ganging med et positivt heltall kan skrives som gjentatt\\ addisjon.\vsk

Ganging med et negativt heltall kan skrives som gjentatt \\ subtraksjon.
}
\eks[1]{ \vs  \label{negeksempel}
\alg{
...
Produktet av et negativt tall og et positivt tall er et negativt tall. \vsk
...
\subsection*{Divisjon}
Definisjonen av divisjon (se \hrs{Divisjon}{seksjon}), kombinert med det vi vet om multiplikasjon med negative tall, gir oss dette:\regv

\st{\begin{center}
		\algv{
			-18:6=\text{''Tallet vi må gange 6 med for å få $ -18 $''}
		}
...
\begin{center}
	\algv{
		42:(-7)=\text{''Tallet vi må gange $ -7 $ med for å få 42''}
	}
	$ (-7)\cdot(-6)=42 $, altså er $ 42:(-6)=-8 $
\end{center}
}\vsk
\st{
\begin{center}
	\algv{
		-45:(-5)=\text{''Tallet vi må gange $ -5 $ med for å få $ -45 $''}
	}
	$ (-5)\cdot9=-45 $, altså er $ -45:(-5)=9 $
\end{center}
 }\vsk
\reg[Divisjon med negative tall]{
Divisjon mellom et positivt tall og et negativt tall gir et\\ negativt tall.\vsk
