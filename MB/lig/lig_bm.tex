...
{\footnotesize \mer I de andre eksemplene har vi valgt å samle $ x $-ene på venstre side av likningen, men vi kan likså gjerne samle dem på høyre side. Ved å gjøre det her har vi unngått utregninger med negative tall.}
}	
...
\eks[6]{ Løs likningen
	...
	\frac{1}{3}x\cdot12+\frac{1}{6}\cdot12&=\frac{5}{12}x\cdot12+2\cdot12\br	
	4x+2 &= 5x+24 \\
	4x-5x &= 24-2 \\
	-x &= 22 \\
...
\[ x^2=9 \]
Dette kalles en \textit{potenslikning}\index{potenslikning}. Potenslikninger er vanligvis vanskelige å løse bare ved hjelp av de fire regneartene, så her må vi også benytte oss av potensregler. Vi opphøyer begge sidene av likningen med den omvendte brøken\footnote{Husk at $ 2=\frac{2}{1} $.} til 2:
...
0 er større enn alle negative tall og mindre enn alle positive tall.	
}
\eks[1]{ \vs
	\[ -9<-8 \]
}
...
Vi kan løse ulikheter ved å bruke metodene fra \rref{lsmlig}, men med ett unntak; \textsl{om vi ganger eller deler med negative tall, skifter ulikheten symbol}. For å forklare hva som skjer, la oss bruke den enkle ulikheten 
\[ 9>8 \]
Om vi ganger begge sider av ulikheten med $ -1 $, får vi $ -9 $ på venstre side og $ -8 $ på høyre side. Men $ {-9 < -8}$. Symbolet fra vår \text{opprinnelige} ulikhet har altså endret seg fra \sym{$>$} til \sym{$<$}. \regv

\reg[Løsing av ulikheter]{
	Ulikheter kan løses på samme måte som likninger, men med ett unntak: Hvis man ganger eller deler begge sider av en ulikhet med et negativt tall, vil $ \sym{$>$} $ endre seg til $ \sym{$<$} $, og omvendt.
}
\eks[1]{
	Løs ulikheten
	\[ 5x-3\geq2x+6 \]
...
\eks[2]{
	...
	\mers{
	Her kan vi selvsagt unngå å dele med $ -2 $ ved å isolere $ x $-ene på høyre side av ulikheten i steden for venstre.	
	} 
}
\section{Likninger med flere ukjente; likningssett}
\spr{
Hvis vi har en likning med flere ukjente, bruker vi uttrykket ''å løse med hensyn på'' for å vise til hvilken av de ukjente vi isolerer på én side av likhetstegnet.
}
\eks[]{
Gitt likningen
\[ 4x+5y=3x+7y+10 \]
...
Hvis vi har to eller flere ukjente tall, er det som regel slik at \regv
...
Fra omskrivingen av \eqref{eks1b} har vi at
\[ x=2y-5 \]
Vi setter løsningen for $ y $ inn i dette uttrykket for $ x $:
\[ x=2y-5=2\cdot 3-5=1 \]
Altså er $ x=1 $ og $ y=3 $.
}
