...
\reg[Brøk som omskriving av et divisjonsstykke \label{brdef}]{
	En brøk\index{brøk} er en annen måte å skrive et divisjonsstykke på. I en brøk kaller vi dividenden for \outl{teller}\index{teller} og divisoren for \outl{nevner}\index{nevner}.
	\fig{br0_bm}
}\vsk
...
på er\!\!
%id{lang}
\footnote{I tillegg har vi utsagnene fra språkboksen på side \pageref{sprakdiv}.}
%\id
\begin{itemize}
	\item ''én firedel''
	\item ''1 av 4''
...
$ \frac{1}{4} $
%\id 
som en mengde. Vi starter med å definere en rute\!\!
%id{def1}
\footnote{Her har vi valgt en enerrute som er større enn den vi brukte i \refkap{Talavare}.}
%\id
 med verdi 1: 
\fig{br1}
Så  deler vi denne ruten inn i fire mindre ruter som er like store. Hver av disse rutene er da
%id{1over4} 
$ \frac{1}{4} $
%\id
(1 av 4):
...
\alg{
	...
	\text{antall blå ruter}+\text{antall tomme ruter}&=\text{nevner} 
}
%\id
\fig{br0a}
}
...
\newpage
\section{Verdi, utviding og forkorting av brøk}
...
\[ \frac{1}{4}=0,25 \]
%\id
(Se \refkap{Utrekning} for hvordan man kan finne at
%id{1div4equals} 
$ 1:4=0,25 $
%\id
\!\!.)
...
Brøker kan ha lik verdi selv om de ser forskjellige ut. De kalles da \outl{likeverdige}\index{likeverdig} brøker. Hvis du regner ut 
%id{1over2}
$ 1:2 $
%\id
, 
...
$ 4:8 $
%\id
\!\!, får du i alle tilfeller 0,5 som svar. Dette betyr at
%id{1o2=2o4=4o8}
\alg{
...
At brøker kan se forskjellige ut, men ha lik verdi, betyr at vi kan\\ endre på utseendet til en brøk uten å endre verdien. La oss som et \\eksempel gjøre om 
%id{3o5}
$ \frac{3}{5} $
%\id
 til en brøk med lik verdi, men med 10 som nevner:
\begin{itemize}
	\item 
	%id{3o5}
	$ \frac{3}{5} $ 
	%\id
	 kan vi gjøre om til en brøk med 10 i nevner om vi deler hver femdel inn i 2 like biter, for da er 1 til sammen delt inn i\\ 
	 %id{5t2eq10}
	 $ {5\cdot2=10} $
	 %\id
	  biter.
	...
	$ \frac{3}{5} $ 
	%\id
	 viser at der er 3 femdeler. Når disse deles i to, blir de totalt til 
	 %id{3t2eq6}
	 $ 3\cdot2=6 $
	 %\id
	  tideler. Altså har 
...
$\frac{6}{10}$ 
%\id
ved å gange både teller og nevner med 2, må vi også kunne gjøre det omvendte. 
%id{6o10}
$ \frac{6}{10} $
%\id
 kan vi skrive om til 
%id{3o5} 
 $\frac{3}{5}$
%\id
 ved å dele både teller og nevner med 2:
 %id{6o10eq3o5}
...
\sv
%id{exp7o3}
\[ \frac{7}{3}=\frac{7\cdot4}{3\cdot4}=\frac{28}{12} \]
...
%id{3o5} 
 $\frac{5}{7}$
%\id 
  har lik eller ulik verdi.

\sv
Vi utvider begge brøkene slik at de har lik nevner. Ved å utvide 
%id{2o3}
$\frac{2}{3}$
%\id
 med 7 og 
...
\reg[\label{brhel}Brøk ganget med heltall]{
...
\newpage
\info{Forkorting ved primtallsfaktorisering}{
...
\info{''Å stryke nuller''}{
	...
	\textsl{Obs!} Null er det eneste sifferet vi kan ''stryke'' slik, for eksempel kan vi ikke forkorte $ \frac{123}{13} $ på noen som helst måte. I tillegg kan vi bare ''stryke'' nuller som står som bakerste siffer, for eksempel kan vi ikke ''stryke'' nuller i brøken $ \frac{101}{10} $.  
}
\newpage
\section{\brdelmbr}
