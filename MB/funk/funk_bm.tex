...
Grafen til $ f(x) $ er nå en tenkt strek som går gjennom alle de uendelig mange punktene vi kan lage av $ x$-verdier og de tilhørende $ f$-verdiene. Vår funksjon er en \textit{lineær}\index{funksjon!lineær} funksjon, noe som betyr at grafen er en rett linje. Altså kan grafen tegnes ved å tegne linja som går gjennom punktene vi har funnet.\vsk
...
\info{$\bm {f(x)}$ og $\bm f$}{
Når vi omtaler en funksjon $f(x)$, nøyer vi oss ofte med å skrive bare $f$. I steden for å skrive for eksempel ${f(x)=2x}$, kunne vi også skrevet $f=x$, men det fine med førstnevnte skrivemåte er at det tydeliggjør at det er $x$ som er variabelen.
}
\info{Navn på $\bm y$-aksen}{
Hvis vi bare har grafen til én funksjon, $f$, plassert i et koordinatsystem, vil vi noen ganger kalle $y$-aksen for $f$-aksen.
\begin{figure}
	\centering
...
For funksjonene $f$ ser vi at om vi står i et punkt på grafen og går 1 steg til høyre, så må vi gå 1 steg ned for å komme tilbake til grafen. Dette betyr at stigningstallet er $\frac{-1}{1}=-1$.\os

For funksjonen $g$ ser vi at om vi står i et punkt på grafen og går 5 skritt til høyre, så må vi gå 3 skritt opp for å komme tilbake til grafen. Dette betyr at stigningstallet er $\frac{3}{5}$.
\fig{funk5a}
}
\newpage
\eks[4]{
	...
	Hvis vi går 3 skritt bort, må vi gå 2 skritt opp for å komme tilbake til grafen. Altså er stigningstallet $\frac{2}{3}$.
}
\newpage
\info{Rette linjer}{
...
\eks{
$f(x)=2x^3$ er en potensfunksjon med koeffisient $k$ og\\ eksponent $3$.
}
