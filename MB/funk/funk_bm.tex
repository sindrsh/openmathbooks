...
\st{
\begin{itemize}
	\item \text{''Antall ruter i \textsl{Figur }$ x $'' er en funksjon av $ x $.}
	\item $ {2x+1} $ er \outl{funksjonsuttrykket}\index{funksjonsuttrykk} til funksjonen ''Antall ruter  i \textsl{Figur }$ x $''.
\end{itemize}	
...
\eks[1]{ \vs
\abc{
\item Gitt funksjonen $f(x)=5x$. Finn $f(3)$.
\item Gitt funksjonen $g(x)=9x+2$. Finn $g(7)$.
\item Gitt funksjonen $h(x)=x^2$. Finn $g(5)$.
}	
\sv \vs
\abc{
\item $f(3)=5\cdot3= 15$
\item $g(7)=9\cdot7+2=65$
\item $h(5)=5^2=25$
}
...
\fig{funk8}	
\abc{
\item Finn uttrykket for $a(x)$.
\item Hvor mange ruter er det når $x=100$?
\item Finn $x$ når $a(x)=47$
}
\sv
\abc{
\item Vi legger merke til at
\begin{itemize}
	\item når $ x=1 $, er det $ 5\cdot1+2=7 $ ruter.
	\item når $ x=2 $, er det $ 5\cdot2+2=12 $ ruter.
	\item når $ x=3 $, er det $ 5\cdot3+2=17 $ ruter.
\end{itemize} 
Altså er 
\[ a(x)=5\cdot x+2 = 5x+2 \]
\item Vi har at
\[ a(100)=5\cdot100+2=500+2=502 \]
Når $x=100$, er det 502 ruter.
\item At $a(x)=47$ gir oss likningen
\alg{
5x+2 &= 47 \\
5x &= 45 \\
x &= 9
...
\item Hvor mange ruter er det når $ x=10 $? \bs
\item Finn $ x $ når $ a(x)=628 $?
}
...
\begin{itemize}
	...
	\item når $ x=4 $, er det $ 4\cdot4+3=19 $ ruter.
\end{itemize} 
Altså er
...
\item At $a(x)=628$ gir oss likningen
\alg{
	x^2+3&=628 \\
	x^2&=625
}
$15\cdot15=625$, altså er
...
\spr{
	Som leseren kanskje har lagt merke til, er en funksjon med et tilhørende funksjonsuttrykk strengt tatt bare en likning med to ukjente. Men ordet \textit{funksjon} brukes i stedet for \textit{likning} for å tydeliggjøre at vi har å gjøre med en likning hvor én variabel er isolert på den éne siden av likhetstegnet, og at vi har et uttrykk av en annen variabel\footnote{En funksjon kan også være uttrykt ved flere variabler.} på den andre siden. Et synonym for \textit{likning} og \textit{funksjon} er \outl{formel}\index{formel}. 
}
\section{\lingraf}
...
\regdef[Potensfunksjoner]{
	Gitt konstantene $ k $ og $ m $. En funksjon på formen
...
Gitt funksjonen $f(x)$ og en konstant $a$. \os
Hvis 
\[ f(x)=ax \]
...
\fork{\ref{funklin} \funklin}{
	...
	Gitt en lineær funksjon $ f(x)=ax+b $ og to forskjellige $ x $-verdier $ x_1 $ og $ x_2 $. Vi setter $ {A=(x_1, b)} $, $ B=(x_2, b) $, $ C=(x_1, f(x_1)) $,\\ $ D=(x_2, f(x_2)) $ og $ E=(0, b) $.
	\fig{funk6a}
	...
	I tillegg er $ {\angle A=\angle B} $, altså oppfyller $ \triangle EAC $ og $ \triangle EBD $ vilkår (iii) fra \rref{vilkform}, og dermed er trekantene formlike. Dette betyr at $ C $ og $ D $ ligger på linje, og denne linja må være grafen til $ f(x) $.
}
