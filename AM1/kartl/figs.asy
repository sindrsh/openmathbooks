...
void db(int I=1, int J=1, int m=I, int n=J, pair sh=(0,0), pen p=blue, pen p1=p, bool l=false, real ls=0.0, real opac=0.1){
	path b = box((0,0),(1,1));
	int cnt = 0;
	for (int i = 1; i <= I; ++i){
		for (int j = 1; j <= J; ++j){
...
void ddb(int I=1, int J=1, int m=I, int n=J, pair sh=(0,0), pen p=purple, bool l=true, real ls=0.0){
	real a = 0.2;
	real h = 0.5;
	path b = box((0,0),(a,h));
	int cnt = 0;
